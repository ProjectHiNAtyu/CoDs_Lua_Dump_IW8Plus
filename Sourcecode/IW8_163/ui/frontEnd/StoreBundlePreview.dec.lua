...
local f0_local14 = function ( f14_arg0, f14_arg1, f14_arg2 )
	...
	local f14_local12 = STORE.GetBundleDescription( f14_arg0._bundleID, f14_local11.totalCP )
	if f14_local12 then
		f14_arg0.DescriptionText:setText( f14_local12 )
	end
	local f14_local13 = StringTable.DIFCHIGDFB( CSV.bundleIDs.file, CSV.bundleIDs.cols.id, f14_arg0._bundleID, CSV.bundleIDs.cols.name )
	if f14_local13 then
		local f14_local14 = Engine.DGAIDIEIAC()
		local f14_local15
		if f14_local14 ~= 0 and f14_local14 ~= 1 then
			f14_local15 = false
		else
			f14_local15 = true
		end
		if not f14_local15 then
			f14_arg0.LocalizedTitle:setText( Engine.CBBHFCGDIC( f14_local13 ) )
		end
	end
	...
	local f14_local15 = LUI.FlowManager.GetScopedData( "MainMenu" )
	if f14_local15 and f14_local15.ShouldHandleGiftBundle then
		f14_local15.ShouldHandleGiftBundle = nil
		f14_arg0:processEvent( {
			name = "bundle_purchase_complete"
		} )
		f14_local7()
