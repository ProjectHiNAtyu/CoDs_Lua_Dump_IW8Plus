...
FenceOnlineServices.init = function ( f41_arg0, f41_arg1 )
	LUI.Fence.init( f41_arg0 )
	f41_arg0._perController = {}
	f41_arg0._noPremium = f41_arg1
	f41_arg0._registrationState = AccountRegistration.BDAJEJBGF()
	local f41_local0 = CODACCOUNT.AccountRegistrationEnabled()
	if f41_local0 then
		f41_local0 = AccountRegistration.BAADBIECDG()
	end
	f41_arg0._autoRegistrationEnabled = f41_local0
	f41_arg0._autoRegistrationState = FenceOnlineServices.AUTO_REGISTRATION_STATE.IDLE
	for f41_local0 = 0, Engine.DCJGJDDEHE() - 1, 1 do
		f41_arg0._perController[f41_local0] = {
			isCancelling = false,
			onlineUpsell = FenceOnlineServices.UPSELL_STATE.inactive,
			fenceState = FenceOnlineServices.CONTROLLER_FENCE_STATE.inactive
		}
		Fences.BHICCDHGCG( f41_local0 )
	end
	if f41_arg1 then
...
FenceOnlineServices.Start = function ( f40_arg0 )
	DebugPrint( "FenceOnlineServices.Start" )
end

FenceOnlineServices.Stop = function ( f39_arg0 )
	DebugPrint( "FenceOnlineServices.Stop" )
	if f39_arg0._openedWaitPopup then
		DebugPrint( "FenceOnlineServices.Stop closing: " .. f39_arg0._openedWaitPopup )
		LUI.FlowManager.RequestLeaveMenuByName( f39_arg0._openedWaitPopup, true )
		f39_arg0._openedWaitPopup = nil
	end
end

FenceOnlineServices.OpenOTPPopup = function ( f35_arg0 )
	if not f35_arg0._openedOTPPopup then
		local f35_local0 = {
			controllerIndex = f35_arg0._registeringController,
			onSuccess = function ()
				f35_arg0._openedOTPPopup = false
			end,
			onCancel = function ()
				f35_arg0._openedOTPPopup = false
			end,
		...
		if CODACCOUNT.AccountRegistrationEnabled() then
			LUI.FlowManager.RequestPopupMenu( nil, "CODAccountAuthentication", true, f35_arg0._registeringController, false, f35_local0, nil, true, true )
			f35_arg0._openedOTPPopup = true
		end
	end
end

FenceOnlineServices.OpenRegisteringPopup = function ( f31_arg0, f31_arg1 )
	if not f31_arg0._openedRegisteringPopup then
		local f31_local0 = {
			controllerIndex = f31_arg0._registeringController,
			isAnonymousAccount = f31_arg1,
			onSuccess = function ()
				f31_arg0._openedRegisteringPopup = false
				f31_arg0._registeringController = nil
				if f31_arg0 then
					f31_arg0._hasSeenGreeting = true
				else
					f31_arg0._openedWaitPopup = nil
				end
			end,
			...
			onCancelFence = function ()
				f31_arg0._registrationCancelled = true
				f31_arg0._registeringController = nil
				f31_arg0._openedRegisteringPopup = false
			end
		}
		if CODACCOUNT.AccountRegistrationEnabled() and not CONDITIONS.IsQABotSkipActive() then
			if not LUI.FlowManager.IsInStack( "CODAccountRegisterPopup" ) then
				LUI.FlowManager.RequestPopupMenu( nil, "CODAccountFencePopup", true, f31_arg0._registeringController, true, f31_local0, nil, true, true )
				f31_arg0._openedRegisteringPopup = true
			end
		else
			if not f31_arg1 then
				AccountRegistration.BAIIEEHFHA( f31_arg0._registeringController )
			end
			f31_local0.onSuccess()
		end
	end
end

FenceOnlineServices.OpenSummaryPopup = function ( f28_arg0 )
	if not f28_arg0._openedSummary and not LUI.FlowManager.IsInStack( "CODAccountRestartingPopup" ) then
		f28_arg0._openedSummary = true
		local f28_local0 = {
			controllerIndex = f28_arg0._summaryController,
			okButtonHandler = function ( f30_arg0, f30_arg1 )
				LUI.FlowManager.RequestLeaveMenuByName( "CODAccountSummary", true, true )
				local f30_local0 = true
				if f28_arg0._summaryController then
					f28_arg0:SetHasSeenCODAccount( f28_arg0._summaryController )
					if Engine.BECCFCBIAA( "hasEverSeen_CODAccountOTP", f28_arg0._summaryController ) ~= 1 then
						if not Dvar.IBEGCHEFE( "MPNNSLPPNP" ) and CODACCOUNT.AccountRegistrationEnabled() and not Engine.DBAEJAHFGJ( f28_arg0._summaryController ) and not TwoFactorAuth.BDAFCCAEDG( f28_arg0._summaryController ) then
							f30_local0 = false
							LUI.FlowManager.RequestPopupMenu( nil, "CODAccountAuthenticationAdv", true, f28_arg0._summaryController, false, {
								state = f28_arg0
							}, nil, true, true )
						else
							f28_arg0:SetHasSeenCODAccountOTP( f28_arg0._summaryController )
						end
					end
				end
				if f30_local0 then
					f28_arg0:SetHasSeenSummary()
				end
			end
		}
		if CODACCOUNT.AccountRegistrationEnabled() and not Engine.DBAEJAHFGJ( f28_arg0._summaryController ) and not CONDITIONS.IsQABotSkipActive() then
			local f28_local1 = Engine.BAGAFBABEE( f28_arg0._summaryController )
			local f28_local2
			if f28_local1 then
				f28_local2 = f28_local1
			else
				f28_local2 = Engine.DCGHGJCHIB( f28_arg0._summaryController, f28_local1 )
			end
			if CONDITIONS.IsLaze() and (f28_local2 or Dvar.IBEGCHEFE( "NRPRRQQSPR" )) then
				LUI.FlowManager.RequestPopupMenu( nil, "CODAccountForceRenamePopup", true, f28_arg0._summaryController, false, {
					controller = f28_arg0._summaryController,
					confirmAction = function ( f29_arg0, f29_arg1 )
						if AccountRegistration.BDBAJHGFE( f29_arg0, f29_arg1 ) then
							Engine.BHIJFFCAEB( "" )
						end
					end
				}, nil, true, true )
			else
				LUI.FlowManager.RequestPopupMenu( nil, "CODAccountSummary", true, f28_arg0._summaryController, false, f28_local0, nil, true, true )
			end
		else
			f28_local0.okButtonHandler()
		end
	end
end

FenceOnlineServices.OpenConnectingPopup = function ( f26_arg0 )
	...
	f26_arg0._openedWaitPopup = "popup_connecting"
end

FenceOnlineServices.OpenQueuedPopup = function ( f24_arg0 )
	local f24_local0 = f24_arg0
	LUI.FlowManager.RequestPopupMenu( nil, "popup_online_queue", false, nil, false, {
		onCancel = function ( f25_arg0, f25_arg1 )
			f24_arg0._perController[f25_arg1].isCancelling = true
		end
	}, nil, true, true )
	f24_arg0._openedWaitPopup = "popup_online_queue"
end

FenceOnlineServices.OpenQueuedRetryPopup = function ( f22_arg0 )
	local f22_local0 = f22_arg0
	LUI.FlowManager.RequestPopupMenu( nil, "popup_online_queue_retry", false, nil, false, {
		onCancel = function ( f23_arg0, f23_arg1 )
			f22_arg0._perController[f23_arg1].isCancelling = true
		end
	}, nil, true, true )
	f22_arg0._openedWaitPopup = "popup_online_queue_retry"
end

FenceOnlineServices.OnBackOut = function ( f21_arg0 )
	DebugPrint( "FenceOnlineServices.OnBackOut" )
	for f21_local0 = 0, Engine.DCJGJDDEHE() - 1, 1 do
		Fences.BHICCDHGCG( f21_local0 )
	end
	if not f21_arg0._noPremium then
...
FenceOnlineServices.GetErrorString = function ( f20_arg0, f20_arg1 )
	local f20_local0 = Fences.GetStatusOnFailure( f20_arg0, FenceOnlineServices.CODE_FENCE )
	if f20_local0 then
		return f20_local0
	elseif f20_arg1 == FenceOnlineServices.ERROR_CODES.networkConnection then
		f20_local0 = "XBOXLIVE/NETCONNECTION"
	elseif f20_arg1 == FenceOnlineServices.ERROR_CODES.signedOutOfSteamOnline then
		f20_local0 = "PLATFORM/STEAM_OFFLINE"
	elseif f20_arg1 == FenceOnlineServices.ERROR_CODES.BattlenetConnected then
		f20_local0 = "XBOXLIVE/BATTLENET_CONNECTION_FAILED"
	elseif f20_arg1 == FenceOnlineServices.ERROR_CODES.LoginQueueClosed then
		f20_local0 = "ONLINE/LOGIN_QUEUE_CLOSED"
	elseif f20_arg1 == FenceOnlineServices.ERROR_CODES.AccountIsBanned then
		f20_local0 = "EXE/ONLINE_PERM_BAN_PLAYER_NO_NAME"
	elseif f20_arg1 == FenceOnlineServices.ERROR_CODES.AccountIsTempBanned then
		f20_local0 = "EXE/ONLINE_TEMP_BAN_PLAYER_NO_NAME"
	elseif f20_arg1 == FenceOnlineServices.ERROR_CODES.AccountIsInMaintenance then
		f20_local0 = "ONLINE/ONLINE_SERVICES_ERROR"
	elseif f20_arg1 == FenceOnlineServices.ERROR_CODES.AccountIsMPOrCPBanned then
		f20_local0 = Engine.CBBHFCGDIC( "EXE/COD_ONLINE_TEMP_BAN_PLAYER", Engine.JCBDICCAH( Engine.DHCFHGIABE( f20_arg0 ) ) ) .. "\n\n" .. Engine.JGAEIIIGD( f20_arg0, 1 )
	end
	assert( f20_local0 )
...
function PopupConnecting( f9_arg0, f9_arg1 )
	...
	MenuUtils.SelectPopupImage( f9_local0 )
...
function PopupConnectingError( f6_arg0, f6_arg1 )
	...
	MenuUtils.SelectPopupImage( f6_local3 )
...
function PopupConnectingCancelled( f5_arg0, f5_arg1 )
	...
	MenuUtils.SelectPopupImage( f5_local0 )
