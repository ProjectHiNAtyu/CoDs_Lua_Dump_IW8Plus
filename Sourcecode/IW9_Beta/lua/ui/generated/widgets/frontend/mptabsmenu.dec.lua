...
require( "ui.generated.widgets.frontend.lfp.lfptopperwidget" )
require( "ui.generated.widgets.frontEnd.mp.PlayMenu.MenuTab" )
local f0_local0 = {
	NonWeaponTypes = {
...
local f0_local2 = function ( f29_arg0, f29_arg1 )
	if f29_arg1 ~= nil then
		...
		if not f29_arg0.bindButton then
			f29_arg0.bindButton = LUI.UIBindButton.new()
			f29_arg0.bindButton.id = "selfBindButton"
			f29_arg0:addElement( f29_arg0.bindButton )
		end
		f29_arg0.bindButton:registerEventHandler( "button_secondary", function ( element, event )
			local f30_local0 = f29_arg0.activeTabWidget
			local f30_local1 = true
			if f30_local0 and f30_local0.BackButtonOverride then
				f30_local1 = f30_local0.BackButtonOverride( element, event )
			end
			if f30_local1 then
				f29_arg0( element, event )
			end
		end )
	end
end
...
local f0_local5 = function ( f25_arg0, f25_arg1, f25_arg2 )
	...
	if f25_arg1 == "PlayMenu" or f25_arg1 == "BRPlayMenu" or f25_arg1 == "CPPlayMenu" then
		Lobby.DisplayNvidiaHighlightsButton( f25_arg0, "MW_Highlights" )
	end
	if (f25_arg1 == "LobbyFlowWidget" or f25_arg1 == "CPPlayMenu" or f25_arg1 == "CPPublicLobbyPlayMenu") and 1 < Lobby.GetCurrentNumLobbyMembers( f25_arg0._controllerIndex ) and Lobby[0xF1D2956B0C99DF6]() then
		f25_arg0._allPlayersMuted = false
		f25_arg0.bindButton:addEventHandler( "button_right_stick", function ( f26_arg0, f26_arg1 )
			f25_arg0._allPlayersMuted = not f25_arg0._allPlayersMuted
			local f26_local0 = f25_arg0._allPlayersMuted and 0x7D144975A6EEA95 or 0x46FABA13E586EDA
			f25_arg0:dispatchEventToAllChildren( {
				name = "lobby_mute_event",
				controllerIndex = controllerIndex
			} )
			Lobby[0xFAAF0691E5D454F]( f26_arg1.controller, f25_arg0._allPlayersMuted )
			return true
		end )
	end
end
...
local f0_local8 = function ( f22_arg0 )
	local f22_local0
	if f22_arg0 ~= "LobbyFlowWidget" and f22_arg0 ~= "CPPlayMenu" and f22_arg0 ~= "CPPublicLobbyPlayMenu" and f22_arg0 ~= "PlayMenu" and f22_arg0 ~= "BRPlayMenu" and f22_arg0 ~= "DMZPlayMenu" then
		f22_local0 = false
	else
		f22_local0 = true
	end
...
local f0_local9 = function ( f16_arg0, f16_arg1, f16_arg2 )
	...
	if TAB.GetOperatorsTabState() ~= TAB.VISIBILITY.HIDDEN then
		local f16_local8
		if CoD.IsDMZMatch() then
			f16_local8 = "OperatorSelectScreen"
		else
	...
	local f16_local11 = TAB.GetBattlepassTabState()
	...
	for f16_local19, f16_local20 in ipairs( f16_local3 ) do
		table.insert( f16_local15, {
			...
			focusFunction = function ( f17_arg0, f17_arg1, f17_arg2, f17_arg3, f17_arg4 )
				...
				if f16_arg0.activeTabIndex and f17_arg2 == f16_arg0.activeTabIndex and f16_arg0.activeTabWidget and not f17_arg4 then
				...
				if f16_arg0._backButtonHandlerSet and f17_arg2 ~= 0 then
					local f17_local2 = f16_arg0.activeTabWidget
					if f17_local2 and not f17_local2.BackButtonOverride then
						f17_local2.BackButtonOverride = function ( f20_arg0, f20_arg1 )
							local f20_local0
				...
				if f16_arg0.menuName == "TournamentLobbyFlow" then
				...
				local f17_local2 = function ( f19_arg0 )
					...
					elseif f17_arg0.isStoreTab and not f17_arg0.isBattlePassTab then
						...
						f19_local1 = ""
					elseif f17_arg0.menuName ~= "LoadoutManagement" and f17_arg0.menuName ~= "OperatorSelectScreen" then
						f19_local0 = Engine[0xED84C33EC5F01EA]( 0x7D9DC9E3291E103 )
						f19_local1 = ""
					end
					...
					elseif f17_arg0.menuName == "MPOperatorMenu" or f17_arg0.menuName == "OperatorSelectScreen" then
					...
					if f17_arg0 == "LobbyFlowWidget" and f17_arg0.menuName ~= f17_arg0 and (Lobby[0x61016770AE2BCA7]() or Engine[0xCBCF315DE9B8AC6]()) then
						f17_arg0.LobbyStatusWidget:ShowStatusWidget( true )
					else
						f17_arg0.LobbyStatusWidget:ShowStatusWidget( false )
					end
				end
...
local f0_local11 = function ( f9_arg0, f9_arg1, f9_arg2 )
	...
	f9_arg0.bindButton:addEventHandler( "button_left_trigger", function ( f13_arg0, f13_arg1 )
		if f9_arg0.Tabs then
			...
			if f9_arg0( f9_arg0, f9_arg0._tabsData[f13_local0.index + 1].menuName ) and 1 < Engine[0x70187FE84AE61FD]() then
				local f13_local1 = LUI.DataSourceInGlobalModel.new( "menu.spltscreenDisplay.controllerIndex" )
				f13_local1:SetValue( f9_arg0, Engine[0x22E32BBCD422B46]( f13_local1:GetValue( f9_arg0 ) ) )
				return true
			end
		end
		return false
...
function MPTabsMenu( menu, controller )
	...
	Tabs.id = "Tabs"
	Tabs:SetAnchorsAndPosition( 0.5, 0.5, 0, 1, _1080p * -516, _1080p * 516, 0, _1080p * 80 )
	...
	LobbyStatusWidget:SetAlpha( 0, 0 )
	LobbyStatusWidget:SetAnchorsAndPosition( 1, 0, 0, 1, _1080p * -512, _1080p * -88, _1080p * 100, _1080p * 290 )
	self:addElement( LobbyStatusWidget )
	self.LobbyStatusWidget = LobbyStatusWidget
	
	local f1_local9 = nil
	if CONDITIONS.IsLFPEnabled() then
		f1_local9 = MenuBuilder.BuildRegisteredType( "LFPTopperWidget", {
			controllerIndex = f1_local1
		} )
		f1_local9.id = "LFPTopperWidget"
		f1_local9:SetAnchorsAndPosition( 1, 0, 0, 1, _1080p * -70.13, _1080p * -20.13, _1080p * 23, _1080p * 73 )
		self:addElement( f1_local9 )
		self.LFPTopperWidget = f1_local9
	end
