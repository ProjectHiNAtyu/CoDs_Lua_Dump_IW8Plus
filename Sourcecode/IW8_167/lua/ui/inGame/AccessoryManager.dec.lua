...
local f0_local0 = function ( f7_arg0, f7_arg1, f7_arg2, f7_arg3, f7_arg4, f7_arg5, f7_arg6 )
	if not MenuBuilder.IsTypeRegistered( f7_arg2 ) then
		...
		f7_local0:ActivateDirtyCheck( WATCH.GetRTTDirtyCheck( f7_arg6 ) == "1" )
		return f7_local0
	end
end
...
function AccessoryManager()
	...
	self.destructDataTurbo = nil
	self:SubscribeToModel( f1_local2, function ( f3_arg0 )
		if self.accessoryUI then
			if self.accessoryUI.Destruct and not Engine.DDJFBBJAIG() then
				self.destructData = self.accessoryUI:Destruct()
			elseif self.accessoryUI.DestructTurbo and not Engine.DDJFBBJAIG() then
				self.destructDataTurbo = self.accessoryUI:DestructTurbo()
			end
		...
		if f3_local0 and 0 <= f3_local0 then
			local f3_local1 = Engine.BBFJAICIBC( self, f3_local0 )
			local f3_local2 = Engine.ECDIACHEID( self, f3_local0 )
			local f3_local3 = Engine.DCCFDBGGEE( self, f3_local0 )
			local f3_local4 = Engine.CBGDIBDFHI( self, f3_local0 )
			if f3_local1 and 0 < #f3_local1 then
				...
				local f3_local6 = PlayerData.BFFBGAJGD( self, CoD.StatsGroup.Common )
				if WATCH.IsPetWatch( f3_local5 ) then
					if WATCH.GetPetWatchType( f3_local5 ) == "pet" then
						self.playerData = f3_local6.petWatchData
					else
						self.playerData = f3_local6.petWatchDataBlack
					end
					self:registerOmnvarHandler( "ui_pet_watch_bonus_earned_0", function ( f5_arg0, f5_arg1 )
						if 80000 < f5_arg1.value and f5_arg1.value < 90000 then
							f5_arg0.playerData.winGamesCharge:set( f5_arg0.playerData.winGamesCharge:get() + f5_arg1.value - 80000 )
							f5_arg0.playerData.dirtyDmg:set( math.max( -240, math.min( 240, f5_arg0.playerData.dirtyDmg:get() + (f5_arg1.value - 80000) * 20 ) ) )
						end
					end )
					self:registerOmnvarHandler( "ui_pet_watch_bonus_earned_1", function ( f4_arg0, f4_arg1 )
						if 90000 < f4_arg1.value and f4_arg1.value < 100000 then
							local f4_local0 = f4_arg0.playerData.winGamesTop3:get()
							f4_arg0.playerData.winGamesTop3:set( f4_arg1.value - 90000 )
						end
					end )
					if not Engine.DDJFBBJAIG() then
						if self.destructData and self.accessoryUI.Reconstruct then
							self.accessoryUI:Reconstruct( self.destructData )
						elseif self.destructDataTurbo and self.accessoryUI.ReconstructTurbo then
							self.accessoryUI:ReconstructTurbo( self.destructDataTurbo )
						elseif self.accessoryUI.Construct then
							self.accessoryUI:Construct()
						end
					end
				elseif WATCH.IsTurboPetWatch( f3_local5 ) then
					if WATCH.GetPetWatchType( f3_local5 ) == "pet_turbo" then
						self.playerData = f3_local6.petWatchDataTurbo
					else
						self.playerData = f3_local6.petWatchDataTurboAlt
					end
					if not Engine.DDJFBBJAIG() then
						if self.destructData and self.accessoryUI.Reconstruct then
							self.accessoryUI:Reconstruct( self.destructData )
	...
	self:SubscribeToModel( DataSources.inGame.player.affectedByEMP:GetModel( f1_local1 ), function ( f2_arg0 )
		if DataModel.JJEHAEBDF( f2_arg0 ) then
