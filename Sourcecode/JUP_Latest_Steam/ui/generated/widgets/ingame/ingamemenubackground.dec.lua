-- Generated by CoDLuaDecompiler
-- Version: 2.4.2
-- Date: 30-11-2025 17:03
module( ..., package.seeall )
local f0_local0 = function ( f2_arg0, f2_arg1, f2_arg2 )
	f2_arg0:addElement( CoD.GetWorldBlur() )
end

function InGameMenuBackground( f1_arg0, f1_arg1 )
	local f1_local0 = CONDITIONS
	local f1_local1 = LUI
	local f1_local2 = MenuBuilder
	local f1_local3 = f1_local1.UIElement.new()
	f1_local3:SetAnchorsAndPosition( 0, 1, 0, 1, 0, 1920, 0, 1080 )
	f1_local3.id = "InGameMenuBackground"
	local f1_local4
	if f1_arg1 then
		f1_local4 = f1_arg1
	else
		f1_local4 = f1_arg1.controllerIndex
	end
	f1_local4 = f1_local1.ValidateControllerIndex( f1_local3, f1_local4 )
	local f1_local5 = nil
	if f1_local0.InGame() then
		f1_local5 = f1_local1.UIImage.new()
		f1_local5[0x8B72E07B55C3AC0] = "InGameDarkener"
		f1_local5:SetRGB( 0, 0 )
		f1_local5:SetAlpha( 0.5, 0 )
		f1_local3:addElement( f1_local5 )
		f1_local3[0xC80BFDAC927BF08] = f1_local5
	end
	f0_local0( f1_local3, f1_local4, f1_arg1 )
	return f1_local3
end

MenuBuilder.registerType( 0xB7F4C9972E116B7, InGameMenuBackground )
LockTable( _M )
