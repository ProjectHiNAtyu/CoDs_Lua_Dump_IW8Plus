...
LUI.ShaderPreloadLayer.init = function ( f17_arg0, f17_arg1 )
	LUI.UIElement.init( f17_arg0 )
	local f17_local0 = f17_arg1.controllerIndex
	if not f17_local0 then
		f17_local0 = Engine.IJEBHJIJF()
	end
	f17_arg0._controllerIndex = f17_local0
	f17_arg0.id = "ShaderPreloadLayer"
	f17_arg0._isShaderPreloadingVisible = false
	f17_arg0._isShaderForceHidden = false
end

LUI.ShaderPreloadLayer.ShouldBeVisible = function ( f16_arg0 )
	local f16_local0 = Engine.DBFFAEEFGJ()
	f16_local0 = f16_local0.flowManager:GetStackTop()
	local f16_local1
	if f16_local0 then
		f16_local1 = f16_local0
	else
		f16_local1 = f16_local0.name == "CinematicFullscreen"
	end
	local f16_local2 = Engine.DIEEIEFCFF()
	if f16_local2 then
		f16_local2 = Dvar.IBEGCHEFE( "MRTNMPRLKR" )
	end
	if not f16_arg0._isShaderPreloadingVisible then
		f16_arg0._isShaderPreloadingVisible = ShaderUpload.CEIBACCEAF()
	end
	local f16_local3 = Engine.CGABICJHAI()
	if f16_local3 then
		f16_local3 = Engine.DDJFBBJAIG()
		if f16_local3 then
			if not f16_local2 then
				f16_local3 = f16_arg0._isShaderPreloadingVisible
				if f16_local3 then
					if not f16_local1 then
						f16_local3 = not f16_arg0._isShaderForceHidden
					else
						f16_local3 = false
					end
				else
...
LUI.ShaderPreloadLayer.GetCurrentState = function ( f15_arg0 )
	local f15_local0 = f15_arg0._currentState
	if not f15_local0 then
		f15_local0 = LUI.ShaderPreloadLayer.States.NONE
	end
	return f15_local0
end

LUI.ShaderPreloadLayer.ForceHideLayer = function ( f14_arg0 )
	local f14_local0 = Engine.DBFFAEEFGJ()
	f14_local0.shaderPreloadLayer._isShaderForceHidden = true
end
