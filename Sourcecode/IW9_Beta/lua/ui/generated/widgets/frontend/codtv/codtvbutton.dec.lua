-- Generated by CoDLuaDecompiler
-- Version: 2.4.2
-- Date: 12-11-2025 23:02
module( ..., package.seeall )
require( "ui.generated.widgets.genericitembackground" )
require( "ui.generated.widgets.genericbuttonselection" )
require( "ui.generated.widgets.genericlistbuttonbackground" )
local f0_local0 = function ( f22_arg0, f22_arg1 )
	f22_arg0.vid_type = MLGContent[0x2BC9C2753BDA1A8]()
	return f22_arg0.vid_type
end

local f0_local1 = function ( f9_arg0, f9_arg1, f9_arg2 )
	f9_arg0.channelIndex = -1
	f9_arg0.vid_type = -1
	f9_arg0.tabChanged = false
	f9_arg0:SubscribeToModelThroughElement( f9_arg0, "name", function ()
		local f21_local0 = f9_arg0.channelIndex
		local f21_local1 = f9_arg0
		local f21_local2 = f9_arg0:GetDataSource()
		f21_local1.channelIndex = f21_local2.index:GetValue( f9_arg0 )
		MLGContent[0x756F1F26602FAAA]( f21_local0, f9_arg0.channelIndex )
		f21_local1 = f9_arg0:GetDataSource()
		f21_local1 = f21_local1.name:GetValue( f9_arg0 )
		if f21_local1 ~= nil then
			f9_arg0.Name:setText( tostring( f21_local1 ) )
		end
	end )
	f9_arg0.Subtitle:SubscribeToModelThroughElement( f9_arg0, "subtitle", function ()
		local f20_local0 = f9_arg0:GetDataSource()
		f20_local0 = f20_local0.subtitle:GetValue( f9_arg0 )
		if f20_local0 ~= nil then
			f9_arg0.Subtitle:setText( tostring( f20_local0 ) )
		end
	end )
	f9_arg0.channel_ID = -1
	f9_arg0:SubscribeToModelThroughElement( f9_arg0, "channel_id", function ()
		local f19_local0 = f9_arg0:GetDataSource()
		f19_local0 = f19_local0.channel_id:GetValue( f9_arg0 )
		if f19_local0 ~= nil then
			f9_arg0.channel_ID = f19_local0
		else
			f9_arg0.channel_ID = -1
		end
	end )
	f9_arg0:addEventHandler( "button_action", function ( f18_arg0, f18_arg1 )
		if f9_arg0.channel_ID ~= -1 then
			MLGContent[0xF6A36D8E8907985]( f9_arg0.vid_type, f9_arg0.channel_ID )
			f9_arg0:dispatchEventToRoot( {
				name = "codtv_video_selected",
				immediate = true,
				controller = f9_arg0,
				video_type = f9_arg0.vid_type,
				index = f9_arg0.channel_ID
			} )
		end
	end )
	f9_arg0:SubscribeToModelThroughElement( f9_arg0, "image", function ()
		local f17_local0 = f9_arg0:GetDataSource()
		f17_local0 = f17_local0.image:GetValue( f9_arg0 )
		if f17_local0 ~= nil and f17_local0 ~= -1 then
			f9_arg0.Image:setImage( RegisterMaterial( "mlg_channel_viewer" .. tostring( f17_local0 ) ), 0 )
		end
	end )
	f9_arg0:SubscribeToModelThroughElement( f9_arg0, "viewers", function ()
		local f16_local0 = f9_arg0( f9_arg0, f9_arg0 )
		local f16_local1 = f9_arg0:GetDataSource()
		f16_local1 = f16_local1.viewers:GetValue( f9_arg0 )
		if f16_local1 ~= nil then
			if f9_arg0.vid_type == CODTV.VIDEO_TYPE.STREAM then
				f9_arg0.ViewsLabel:setText( "" )
			elseif 0 < f16_local1 then
				f9_arg0.ViewsLabel:setText( Engine[0xED84C33EC5F01EA]( 0x51B4EEA2C2F4FB7, f16_local1 ) )
			else
				f9_arg0.ViewsLabel:setText( Engine[0xED84C33EC5F01EA]( "" ) )
			end
		end
	end )
	f9_arg0:SubscribeToModelThroughElement( f9_arg0, "last_updated", function ()
		local f15_local0 = f9_arg0:GetDataSource()
		f15_local0 = f15_local0.last_updated:GetValue( f9_arg0 )
		if f15_local0 ~= nil then
			f9_arg0.DateLabel:setText( f15_local0 )
		end
	end )
	f9_arg0:SubscribeToModelThroughElement( f9_arg0, "length", function ()
		local f14_local0 = f9_arg0:GetDataSource()
		f14_local0 = f14_local0.length:GetValue( f9_arg0 )
		if f14_local0 ~= nil and f9_arg0( f9_arg0, f9_arg0 ) == CODTV.VIDEO_TYPE.VOD then
			f9_arg0.DurationBG:SetAlpha( 0.6 )
			f9_arg0.DurationLabel:SetAlpha( 1 )
			f9_arg0.DurationLabel:setText( f14_local0 )
		else
			f9_arg0.DurationBG:SetAlpha( 0 )
			f9_arg0.DurationLabel:SetAlpha( 0 )
		end
	end )
	f9_arg0:SubscribeToModelThroughElement( f9_arg0, "status", function ()
		local f13_local0 = f9_arg0( f9_arg0, f9_arg0 )
		if f9_arg0.vid_type == CODTV.VIDEO_TYPE.STREAM then
			local f13_local1 = f9_arg0:GetDataSource()
			if f13_local1.status:GetValue( f9_arg0 ) == 1 then
				f9_arg0.LiveLabel:setText( Engine[0xED84C33EC5F01EA]( 0xB00A7A7D3042D49 ), 0 )
				f9_arg0.LiveIcon:SetRGBFromInt( 16318464, 0 )
			else
				f9_arg0.LiveIcon:SetRGBFromInt( 16776960, 0 )
				f9_arg0.LiveLabel:setText( Engine[0xED84C33EC5F01EA]( 0x1DB8EBD021A341D ), 0 )
			end
		end
	end )
	f9_arg0:addEventHandler( "button_over", function ( f12_arg0, f12_arg1 )
		local f12_local0 = f9_arg0( f9_arg0, f12_arg1.controller or f9_arg0 )
		if f9_arg0.vid_type == CODTV.VIDEO_TYPE.STREAM then
			ACTIONS.AnimateSequence( f9_arg0, "ButtonOverStream" )
		else
			ACTIONS.AnimateSequence( f9_arg0, "ButtonOverVOD" )
		end
		f9_arg0.tabChanged = false
	end )
	f9_arg0:addEventHandler( "button_up", function ( f11_arg0, f11_arg1 )
		local f11_local0 = f11_arg1.controller or f9_arg0
		if f9_arg0.vid_type == CODTV.VIDEO_TYPE.STREAM then
			if f9_arg0.tabChanged then
				ACTIONS.AnimateSequence( f9_arg0, "ButtonResetStream" )
			else
				ACTIONS.AnimateSequence( f9_arg0, "ButtonUpStream" )
			end
		else
			if f9_arg0.tabChanged then
				ACTIONS.AnimateSequence( f9_arg0, "ButtonResetVOD" )
			else
				ACTIONS.AnimateSequence( f9_arg0, "ButtonUpVOD" )
			end
			f9_arg0.tabChanged = false
		end
	end )
	f9_arg0:addEventHandler( "codtv_tab_changed", function ( f10_arg0, f10_arg1 )
		f9_arg0.channelIndex = -1
		f9_arg0.tabChanged = true
	end )
end

function CODTvButton( menu, controller )
	local self = LUI.UIButton.new()
	self:SetAnchorsAndPosition( 0, 1, 0, 1, 0, 300 * _1080p, 0, 145 * _1080p )
	self.id = "CODTvButton"
	self._animationSets = self._animationSets or {}
	self._sequences = self._sequences or {}
	self._sequenceLengths = self._sequenceLengths or {}
	local f1_local1
	if controller then
		f1_local1 = controller
	else
		f1_local1 = controller.controllerIndex
	end
	f1_local1 = LUI.ValidateControllerIndex( self, f1_local1 )
	local f1_local2 = self
	local GenericItemBackground = nil
	
	GenericItemBackground = MenuBuilder.BuildRegisteredType( "GenericItemBackground", {
		controllerIndex = f1_local1
	} )
	GenericItemBackground.id = "GenericItemBackground"
	GenericItemBackground:SetAnchorsAndPosition( 0, 0, 0, 0, 0, 0, _1080p * 24, 0 )
	self:addElement( GenericItemBackground )
	self.GenericItemBackground = GenericItemBackground
	
	local DropShadow = nil
	
	DropShadow = LUI.UIImage.new()
	DropShadow.id = "DropShadow"
	DropShadow:SetRGBFromInt( 0, 0 )
	DropShadow:SetAlpha( 0.3, 0 )
	DropShadow:setImage( RegisterMaterial( "widg_gradient_top_to_bottom" ), 0 )
	DropShadow:SetAnchorsAndPosition( 0, 0, 1, 0, 0, 0, 0, _1080p * 4 )
	self:addElement( DropShadow )
	self.DropShadow = DropShadow
	
	local Image = nil
	
	Image = LUI.UIImage.new()
	Image.id = "Image"
	Image:SetAnchorsAndPosition( 0, 1, 0, 1, 0, _1080p * 208, _1080p * 28, _1080p * 145 )
	self:addElement( Image )
	self.Image = Image
	
	local GenericButtonSelection = nil
	
	GenericButtonSelection = MenuBuilder.BuildRegisteredType( "GenericButtonSelection", {
		controllerIndex = f1_local1
	} )
	GenericButtonSelection.id = "GenericButtonSelection"
	GenericButtonSelection:SetAnchorsAndPosition( 0, 0, 0, 0, 0, 0, _1080p * 24, _1080p * 2 )
	self:addElement( GenericButtonSelection )
	self.GenericButtonSelection = GenericButtonSelection
	
	local TitleBackground = nil
	
	TitleBackground = MenuBuilder.BuildRegisteredType( "GenericListButtonBackground", {
		controllerIndex = f1_local1
	} )
	TitleBackground.id = "TitleBackground"
	TitleBackground.DropShadow:SetAlpha( 0, 0 )
	TitleBackground:SetAnchorsAndPosition( 0, 0, 0, 1, 0, 0, 0, _1080p * 24 )
	self:addElement( TitleBackground )
	self.TitleBackground = TitleBackground
	
	local Name = nil
	
	Name = LUI.UIText.new()
	Name.id = "Name"
	Name:SetRGBFromTable( SWATCHES.genericButton.textFocus, 0 )
	Name:setText( "Item Name", 0 )
	Name:SetFont( FONTS.GetFont( FONTS.PrimaryRegular.File ) )
	Name:SetFontSize( 18 * _PT )
	Name:SetMinFontSize( 18 * _PT )
	Name:SetWordWrap( true )
	Name:SetAlignment( LUI.Alignment.Right )
	Name:SetAnchorsAndPosition( 1, 0, 0, 1, _1080p * -287, _1080p * -13, _1080p * 2.5, _1080p * 21.5 )
	self:addElement( Name )
	self.Name = Name
	
	local SubtitleBG = nil
	
	SubtitleBG = LUI.UIImage.new()
	SubtitleBG.id = "SubtitleBG"
	SubtitleBG:SetAlpha( 0, 0 )
	SubtitleBG:SetAnchorsAndPosition( 0, 1, 0, 1, _1080p * 262.19, _1080p * 476.5, _1080p * 43.5, _1080p * 64.5 )
	self:addElement( SubtitleBG )
	self.SubtitleBG = SubtitleBG
	
	local Subtitle = nil
	
	Subtitle = LUI.UIText.new()
	Subtitle.id = "Subtitle"
	Subtitle:SetRGBFromInt( 0, 0 )
	Subtitle:SetAlpha( 0, 0 )
	Subtitle:setText( "STREAMER NAME", 0 )
	Subtitle:SetFont( FONTS.GetFont( FONTS.PrimaryRegular.File ) )
	Subtitle:SetFontSize( 18 * _PT )
	Subtitle:SetMinFontSize( 18 * _PT )
	Subtitle:SetWordWrap( true )
	Subtitle:SetAlignment( LUI.Alignment.Left )
	Subtitle:SetAnchorsAndPosition( 0, 1, 0, 1, _1080p * 267.76, _1080p * 466.5, _1080p * 44, _1080p * 62 )
	self:addElement( Subtitle )
	self.Subtitle = Subtitle
	
	local SubtitleInfoCap = nil
	
	SubtitleInfoCap = LUI.UIImage.new()
	SubtitleInfoCap.id = "SubtitleInfoCap"
	SubtitleInfoCap:SetRGBFromInt( 14277081, 0 )
	SubtitleInfoCap:SetAlpha( 0, 0 )
	SubtitleInfoCap:SetAnchorsAndPosition( 0, 1, 0, 1, _1080p * 255, _1080p * 258, _1080p * 43.5, _1080p * 72.5 )
	self:addElement( SubtitleInfoCap )
	self.SubtitleInfoCap = SubtitleInfoCap
	
	local ViewsLabel = nil
	
	ViewsLabel = LUI.UIText.new()
	ViewsLabel.id = "ViewsLabel"
	ViewsLabel:SetAlpha( 0, 0 )
	ViewsLabel:setText( Engine[0xED84C33EC5F01EA]( 0x9DD620DECA998AE ), 0 )
	ViewsLabel:SetFont( FONTS.GetFont( FONTS.PrimaryRegular.File ) )
	ViewsLabel:SetFontSize( 18 * _PT )
	ViewsLabel:SetMinFontSize( 18 * _PT )
	ViewsLabel:SetWordWrap( true )
	ViewsLabel:SetAlignment( LUI.Alignment.Left )
	ViewsLabel:SetAnchorsAndPosition( 0, 1, 0, 1, _1080p * 262.19, _1080p * 476.5, _1080p * 95, _1080p * 113 )
	self:addElement( ViewsLabel )
	self.ViewsLabel = ViewsLabel
	
	local LiveIcon = nil
	
	LiveIcon = LUI.UIImage.new()
	LiveIcon.id = "LiveIcon"
	LiveIcon:SetRGBFromInt( 16318464, 0 )
	LiveIcon:SetAlpha( 0, 0 )
	LiveIcon:setImage( RegisterMaterial( "widg_circle_small" ), 0 )
	LiveIcon:SetAnchorsAndPosition( 0, 1, 0, 1, _1080p * 262.19, _1080p * 270.19, _1080p * 77.5, _1080p * 85.5 )
	self:addElement( LiveIcon )
	self.LiveIcon = LiveIcon
	
	local LiveLabel = nil
	
	LiveLabel = LUI.UIText.new()
	LiveLabel.id = "LiveLabel"
	LiveLabel:SetAlpha( 0, 0 )
	LiveLabel:setText( Engine[0xED84C33EC5F01EA]( 0xB00A7A7D3042D49 ), 0 )
	LiveLabel:SetFont( FONTS.GetFont( FONTS.PrimaryRegular.File ) )
	LiveLabel:SetFontSize( 18 * _PT )
	LiveLabel:SetMinFontSize( 18 * _PT )
	LiveLabel:SetWordWrap( true )
	LiveLabel:SetAlignment( LUI.Alignment.Left )
	LiveLabel:SetAnchorsAndPosition( 0, 1, 0, 1, _1080p * 275, _1080p * 476.5, _1080p * 72.5, _1080p * 90.5 )
	self:addElement( LiveLabel )
	self.LiveLabel = LiveLabel
	
	local DateLabel = nil
	
	DateLabel = LUI.UIText.new()
	DateLabel.id = "DateLabel"
	DateLabel:SetRGBFromInt( 0, 0 )
	DateLabel:SetAlpha( 0, 0 )
	DateLabel:setText( "", 0 )
	DateLabel:SetFont( FONTS.GetFont( FONTS.PrimaryRegular.File ) )
	DateLabel:SetFontSize( 18 * _PT )
	DateLabel:SetMinFontSize( 18 * _PT )
	DateLabel:SetWordWrap( true )
	DateLabel:SetAlignment( LUI.Alignment.Left )
	DateLabel:SetAnchorsAndPosition( 0, 1, 0, 1, _1080p * 267.76, _1080p * 466.5, _1080p * 44, _1080p * 62 )
	self:addElement( DateLabel )
	self.DateLabel = DateLabel
	
	local DurationBG = nil
	
	DurationBG = LUI.UIImage.new()
	DurationBG.id = "DurationBG"
	DurationBG:SetRGBFromInt( 0, 0 )
	DurationBG:SetAlpha( 0, 0 )
	DurationBG:SetAnchorsAndPosition( 0, 1, 0, 1, _1080p * 149, _1080p * 208, _1080p * 122, _1080p * 145 )
	self:addElement( DurationBG )
	self.DurationBG = DurationBG
	
	local DurationLabel = nil
	
	DurationLabel = LUI.UIText.new()
	DurationLabel.id = "DurationLabel"
	DurationLabel:SetAlpha( 0, 0 )
	DurationLabel:setText( "09:52:10", 0 )
	DurationLabel:SetFont( FONTS.GetFont( FONTS.PrimaryRegular.File ) )
	DurationLabel:SetFontSize( 18 * _PT )
	DurationLabel:SetMinFontSize( 18 * _PT )
	DurationLabel:SetWordWrap( true )
	DurationLabel:SetAlignment( LUI.Alignment.Center )
	DurationLabel:SetAnchorsAndPosition( 0, 1, 0, 1, _1080p * 156.5, _1080p * 200.5, _1080p * 125.5, _1080p * 141.5 )
	self:addElement( DurationLabel )
	self.DurationLabel = DurationLabel
	
	self._sequences.DefaultSequence = function ()
		
	end
	
	self._sequenceLengths.DefaultSequence = 0
	local f1_local18 = nil
	GenericItemBackground:RegisterAnimationSequence( "ButtonOverStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 24
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 200
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 24
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	DropShadow:RegisterAnimationSequence( "ButtonOverStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 4
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 200
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 4
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	GenericButtonSelection:RegisterAnimationSequence( "ButtonOverStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 24
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 2
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 200
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 24
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 2
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	TitleBackground:RegisterAnimationSequence( "ButtonOverStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 24
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 200
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 24
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	Name:RegisterAnimationSequence( "ButtonOverStream", {
		{
			duration = 0,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * -287
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 21.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 2.5
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.RGB
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * -287
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 187
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 2.5
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 21.5
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	SubtitleBG:RegisterAnimationSequence( "ButtonOverStream", {
		{
			duration = 0,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * 262.19
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 266.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 43.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 64.5
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 64.5
		},
		{
			value = 0,
			duration = -48,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 476.5
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * 262.19
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 43.5
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 1,
			duration = -58,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	Subtitle:RegisterAnimationSequence( "ButtonOverStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 0,
			duration = -48,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 1,
			duration = -58,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	SubtitleInfoCap:RegisterAnimationSequence( "ButtonOverStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	ViewsLabel:RegisterAnimationSequence( "ButtonOverStream", {
		{
			duration = 0,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * 262.19
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 476.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 95
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 113
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 0,
			duration = -48,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 1,
			duration = -58,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	LiveIcon:RegisterAnimationSequence( "ButtonOverStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 0,
			duration = -48,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 1,
			duration = -58,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	LiveLabel:RegisterAnimationSequence( "ButtonOverStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 0,
			duration = -48,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 1,
			duration = -58,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	self._sequences.ButtonOverStream = function ()
		menu:AnimateSequence( "ButtonOverStream" )
		menu:AnimateSequence( "ButtonOverStream" )
		menu:AnimateSequence( "ButtonOverStream" )
		menu:AnimateSequence( "ButtonOverStream" )
		menu:AnimateSequence( "ButtonOverStream" )
		menu:AnimateSequence( "ButtonOverStream" )
		menu:AnimateSequence( "ButtonOverStream" )
		menu:AnimateSequence( "ButtonOverStream" )
		menu:AnimateSequence( "ButtonOverStream" )
		menu:AnimateSequence( "ButtonOverStream" )
		menu:AnimateSequence( "ButtonOverStream" )
	end
	
	self._sequenceLengths.ButtonOverStream = 150
	local f1_local19 = nil
	GenericItemBackground:RegisterAnimationSequence( "ButtonUpStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 200
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 24
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Right
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 24
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	DropShadow:RegisterAnimationSequence( "ButtonUpStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 200
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 4
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Right
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 4
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	GenericButtonSelection:RegisterAnimationSequence( "ButtonUpStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 200
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 24
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 2
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Right
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 24
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 2
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	TitleBackground:RegisterAnimationSequence( "ButtonUpStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 200
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 24
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Right
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 24
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	Name:RegisterAnimationSequence( "ButtonUpStream", {
		{
			duration = 0,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * -287
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 21.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 2.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 187
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.RGB,
			value = SWATCHES.genericButton.textFocus
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * -287
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * -13
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 2.5
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 21.5
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	SubtitleBG:RegisterAnimationSequence( "ButtonUpStream", {
		{
			duration = 0,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * 262.19
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 64.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 43.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 476.5
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * 262.19
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 276.5
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 43.5
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 64.5
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	Subtitle:RegisterAnimationSequence( "ButtonUpStream", {
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 0,
			duration = -48,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	SubtitleInfoCap:RegisterAnimationSequence( "ButtonUpStream", {
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	ViewsLabel:RegisterAnimationSequence( "ButtonUpStream", {
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * 262.19
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 476.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 95
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 113
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	LiveIcon:RegisterAnimationSequence( "ButtonUpStream", {
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	LiveLabel:RegisterAnimationSequence( "ButtonUpStream", {
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	DateLabel:RegisterAnimationSequence( "ButtonUpStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	self._sequences.ButtonUpStream = function ()
		menu:AnimateSequence( "ButtonUpStream" )
		menu:AnimateSequence( "ButtonUpStream" )
		menu:AnimateSequence( "ButtonUpStream" )
		menu:AnimateSequence( "ButtonUpStream" )
		menu:AnimateSequence( "ButtonUpStream" )
		menu:AnimateSequence( "ButtonUpStream" )
		menu:AnimateSequence( "ButtonUpStream" )
		menu:AnimateSequence( "ButtonUpStream" )
		menu:AnimateSequence( "ButtonUpStream" )
		menu:AnimateSequence( "ButtonUpStream" )
		menu:AnimateSequence( "ButtonUpStream" )
		menu:AnimateSequence( "ButtonUpStream" )
	end
	
	self._sequenceLengths.ButtonUpStream = 150
	local f1_local20 = nil
	GenericItemBackground:RegisterAnimationSequence( "ButtonOverVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 24
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 200
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 24
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	DropShadow:RegisterAnimationSequence( "ButtonOverVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 4
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 200
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 4
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	GenericButtonSelection:RegisterAnimationSequence( "ButtonOverVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 24
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 2
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 200
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 24
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 2
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	TitleBackground:RegisterAnimationSequence( "ButtonOverVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 24
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 200
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 24
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	Name:RegisterAnimationSequence( "ButtonOverVOD", {
		{
			duration = 0,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * -287
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 21.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 2.5
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.RGB
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * -287
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 187
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 2.5
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 21.5
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	SubtitleBG:RegisterAnimationSequence( "ButtonOverVOD", {
		{
			duration = 0,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * 262.19
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 266.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 43.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 64.5
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 64.5
		},
		{
			value = 0,
			duration = -48,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 476.5
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * 262.19
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 43.5
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 1,
			duration = -58,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	SubtitleInfoCap:RegisterAnimationSequence( "ButtonOverVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	ViewsLabel:RegisterAnimationSequence( "ButtonOverVOD", {
		{
			duration = 0,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * 262.19
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 476.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 72.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 90.5
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 90.5
		},
		{
			value = 0,
			duration = -48,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 476.5
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * 262.19
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 72.5
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 1,
			duration = -58,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	DateLabel:RegisterAnimationSequence( "ButtonOverVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 0,
			duration = -48,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 1,
			duration = -58,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	self._sequences.ButtonOverVOD = function ()
		menu:AnimateSequence( "ButtonOverVOD" )
		menu:AnimateSequence( "ButtonOverVOD" )
		menu:AnimateSequence( "ButtonOverVOD" )
		menu:AnimateSequence( "ButtonOverVOD" )
		menu:AnimateSequence( "ButtonOverVOD" )
		menu:AnimateSequence( "ButtonOverVOD" )
		menu:AnimateSequence( "ButtonOverVOD" )
		menu:AnimateSequence( "ButtonOverVOD" )
		menu:AnimateSequence( "ButtonOverVOD" )
	end
	
	self._sequenceLengths.ButtonOverVOD = 150
	local f1_local21 = nil
	GenericItemBackground:RegisterAnimationSequence( "ButtonUpVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 200
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 24
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Right
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 24
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	DropShadow:RegisterAnimationSequence( "ButtonUpVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 200
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 4
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Right
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 4
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	GenericButtonSelection:RegisterAnimationSequence( "ButtonUpVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 200
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 24
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 2
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Right
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 24
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 2
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	TitleBackground:RegisterAnimationSequence( "ButtonUpVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 200
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 24
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Right
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 24
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	Name:RegisterAnimationSequence( "ButtonUpVOD", {
		{
			duration = 0,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * -287
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 21.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 2.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 187
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.RGB,
			value = SWATCHES.genericButton.textFocus
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * -287
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * -13
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 2.5
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 21.5
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	SubtitleBG:RegisterAnimationSequence( "ButtonUpVOD", {
		{
			duration = 0,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * 262.19
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 64.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 43.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 476.5
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * 262.19
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 276.5
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 43.5
		},
		{
			duration = 150,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 64.5
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 1,
			duration = 150,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	SubtitleInfoCap:RegisterAnimationSequence( "ButtonUpVOD", {
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 0,
			duration = 150,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	ViewsLabel:RegisterAnimationSequence( "ButtonUpVOD", {
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * 262.19
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 476.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 72.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 90.5
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		},
		{
			value = 0,
			duration = -48,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	LiveIcon:RegisterAnimationSequence( "ButtonUpVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	LiveLabel:RegisterAnimationSequence( "ButtonUpVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	DateLabel:RegisterAnimationSequence( "ButtonUpVOD", {
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 0,
			duration = -48,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	self._sequences.ButtonUpVOD = function ()
		menu:AnimateSequence( "ButtonUpVOD" )
		menu:AnimateSequence( "ButtonUpVOD" )
		menu:AnimateSequence( "ButtonUpVOD" )
		menu:AnimateSequence( "ButtonUpVOD" )
		menu:AnimateSequence( "ButtonUpVOD" )
		menu:AnimateSequence( "ButtonUpVOD" )
		menu:AnimateSequence( "ButtonUpVOD" )
		menu:AnimateSequence( "ButtonUpVOD" )
		menu:AnimateSequence( "ButtonUpVOD" )
		menu:AnimateSequence( "ButtonUpVOD" )
		menu:AnimateSequence( "ButtonUpVOD" )
	end
	
	self._sequenceLengths.ButtonUpVOD = 150
	local f1_local22 = nil
	GenericItemBackground:RegisterAnimationSequence( "ButtonResetVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 24
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	DropShadow:RegisterAnimationSequence( "ButtonResetVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 4
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	GenericButtonSelection:RegisterAnimationSequence( "ButtonResetVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 24
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 2
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	TitleBackground:RegisterAnimationSequence( "ButtonResetVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 24
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	Name:RegisterAnimationSequence( "ButtonResetVOD", {
		{
			duration = 0,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * -287
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * -13
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 2.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 21.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.RGB,
			value = SWATCHES.genericButton.textFocus
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	SubtitleBG:RegisterAnimationSequence( "ButtonResetVOD", {
		{
			duration = 0,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * 262.19
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 276.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 43.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 64.5
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	Subtitle:RegisterAnimationSequence( "ButtonResetVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	SubtitleInfoCap:RegisterAnimationSequence( "ButtonResetVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	ViewsLabel:RegisterAnimationSequence( "ButtonResetVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	LiveIcon:RegisterAnimationSequence( "ButtonResetVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	LiveLabel:RegisterAnimationSequence( "ButtonResetVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	DateLabel:RegisterAnimationSequence( "ButtonResetVOD", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	self._sequences.ButtonResetVOD = function ()
		menu:AnimateSequence( "ButtonResetVOD" )
		menu:AnimateSequence( "ButtonResetVOD" )
		menu:AnimateSequence( "ButtonResetVOD" )
		menu:AnimateSequence( "ButtonResetVOD" )
		menu:AnimateSequence( "ButtonResetVOD" )
		menu:AnimateSequence( "ButtonResetVOD" )
		menu:AnimateSequence( "ButtonResetVOD" )
		menu:AnimateSequence( "ButtonResetVOD" )
		menu:AnimateSequence( "ButtonResetVOD" )
		menu:AnimateSequence( "ButtonResetVOD" )
		menu:AnimateSequence( "ButtonResetVOD" )
		menu:AnimateSequence( "ButtonResetVOD" )
	end
	
	self._sequenceLengths.ButtonResetVOD = 0
	local f1_local23 = nil
	GenericItemBackground:RegisterAnimationSequence( "ButtonResetStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 24
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	DropShadow:RegisterAnimationSequence( "ButtonResetStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 4
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	GenericButtonSelection:RegisterAnimationSequence( "ButtonResetStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 24
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 2
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	TitleBackground:RegisterAnimationSequence( "ButtonResetStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 24
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	Name:RegisterAnimationSequence( "ButtonResetStream", {
		{
			duration = 0,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * -287
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * -13
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 2.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 21.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.RGB,
			value = SWATCHES.genericButton.textFocus
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	SubtitleBG:RegisterAnimationSequence( "ButtonResetStream", {
		{
			duration = 0,
			property = TWEEN_PROPERTY.Left,
			value = _1080p * 262.19
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Right,
			value = _1080p * 276.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Top,
			value = _1080p * 43.5
		},
		{
			duration = 0,
			property = TWEEN_PROPERTY.Bottom,
			value = _1080p * 64.5
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Left_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Right_Anchor
		},
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Top_Anchor
		},
		{
			value = 1,
			duration = 0,
			property = TWEEN_PROPERTY.Bottom_Anchor
		}
	} )
	Subtitle:RegisterAnimationSequence( "ButtonResetStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	SubtitleInfoCap:RegisterAnimationSequence( "ButtonResetStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	ViewsLabel:RegisterAnimationSequence( "ButtonResetStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	LiveIcon:RegisterAnimationSequence( "ButtonResetStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	LiveLabel:RegisterAnimationSequence( "ButtonResetStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	DateLabel:RegisterAnimationSequence( "ButtonResetStream", {
		{
			value = 0,
			duration = 0,
			property = TWEEN_PROPERTY.Alpha
		}
	} )
	self._sequences.ButtonResetStream = function ()
		menu:AnimateSequence( "ButtonResetStream" )
		menu:AnimateSequence( "ButtonResetStream" )
		menu:AnimateSequence( "ButtonResetStream" )
		menu:AnimateSequence( "ButtonResetStream" )
		menu:AnimateSequence( "ButtonResetStream" )
		menu:AnimateSequence( "ButtonResetStream" )
		menu:AnimateSequence( "ButtonResetStream" )
		menu:AnimateSequence( "ButtonResetStream" )
		menu:AnimateSequence( "ButtonResetStream" )
		menu:AnimateSequence( "ButtonResetStream" )
		menu:AnimateSequence( "ButtonResetStream" )
		menu:AnimateSequence( "ButtonResetStream" )
	end
	
	self._sequenceLengths.ButtonResetStream = 0
	f0_local0( self, f1_local1, controller )
	return self
end

MenuBuilder.registerType( "CODTvButton", CODTvButton )
LockTable( _M )
