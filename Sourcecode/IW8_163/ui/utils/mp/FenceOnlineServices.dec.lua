...
FenceOnlineServices.init = function ( f40_arg0, f40_arg1 )
	LUI.Fence.init( f40_arg0 )
	f40_arg0._perController = {}
	f40_arg0._noPremium = f40_arg1
	f40_arg0._registrationState = AccountRegistration.BDAJEJBGF()
	local f40_local0 = CODACCOUNT.AccountRegistrationEnabled()
	if f40_local0 then
		f40_local0 = AccountRegistration.BAADBIECDG()
	end
	f40_arg0._autoRegistrationEnabled = f40_local0
	f40_arg0._autoRegistrationState = FenceOnlineServices.AUTO_REGISTRATION_STATE.IDLE
	for f40_local0 = 0, Engine.DCJGJDDEHE() - 1, 1 do
		f40_arg0._perController[f40_local0] = {
			isCancelling = false,
			onlineUpsell = FenceOnlineServices.UPSELL_STATE.inactive,
			fenceState = FenceOnlineServices.CONTROLLER_FENCE_STATE.inactive
		}
		Fences.BHICCDHGCG( f40_local0 )
	end
	if f40_arg1 then
...
FenceOnlineServices.Start = function ( f39_arg0 )
	DebugPrint( "FenceOnlineServices.Start" )
end

FenceOnlineServices.Stop = function ( f38_arg0 )
	DebugPrint( "FenceOnlineServices.Stop" )
	if f38_arg0._openedWaitPopup then
		DebugPrint( "FenceOnlineServices.Stop closing: " .. f38_arg0._openedWaitPopup )
		LUI.FlowManager.RequestLeaveMenuByName( f38_arg0._openedWaitPopup, true )
		f38_arg0._openedWaitPopup = nil
	end
end

FenceOnlineServices.OpenOTPPopup = function ( f34_arg0 )
	if not f34_arg0._openedOTPPopup then
		local f34_local0 = {
			controllerIndex = f34_arg0._registeringController,
			onSuccess = function ()
				f34_arg0._openedOTPPopup = false
			end,
			onCancel = function ()
				f34_arg0._openedOTPPopup = false
			end,
		...
		if CODACCOUNT.AccountRegistrationEnabled() then
			LUI.FlowManager.RequestPopupMenu( nil, "CODAccountAuthentication", true, f34_arg0._registeringController, false, f34_local0, nil, true, true )
			f34_arg0._openedOTPPopup = true
		end
	end
end

FenceOnlineServices.OpenRegisteringPopup = function ( f30_arg0, f30_arg1 )
	if not f30_arg0._openedRegisteringPopup then
		local f30_local0 = {
			controllerIndex = f30_arg0._registeringController,
			isAnonymousAccount = f30_arg1,
			onSuccess = function ()
				f30_arg0._openedRegisteringPopup = false
				f30_arg0._registeringController = nil
				if f30_arg0 then
					f30_arg0._hasSeenGreeting = true
				else
					f30_arg0._openedWaitPopup = nil
				end
			end,
			...
			onCancelFence = function ()
				f30_arg0._registrationCancelled = true
				f30_arg0._registeringController = nil
				f30_arg0._openedRegisteringPopup = false
			end
		}
		if CODACCOUNT.AccountRegistrationEnabled() and not CONDITIONS.IsQABotSkipActive() then
			if not LUI.FlowManager.IsInStack( "CODAccountRegisterPopup" ) then
				LUI.FlowManager.RequestPopupMenu( nil, "CODAccountFencePopup", true, f30_arg0._registeringController, true, f30_local0, nil, true, true )
				f30_arg0._openedRegisteringPopup = true
			end
		else
			if not f30_arg1 then
				AccountRegistration.BAIIEEHFHA( f30_arg0._registeringController )
			end
			f30_local0.onSuccess()
		end
	end
end
...
FenceOnlineServices.OpenSummaryPopup = function ( f28_arg0 )
	if not f28_arg0._openedSummary and not LUI.FlowManager.IsInStack( "CODAccountRestartingPopup" ) then
		...
		local f28_local0 = {
			...
			okButtonHandler = function ( f29_arg0, f29_arg1 )
