...
BR.GetUnpackedKeypadData = function ( f15_arg0 )
	local f15_local0, f15_local1 = nil
	local f15_local2 = {}
	local f15_local3 = DataSources.inGame.player.BRData.keypadPackedData:GetValue( f15_arg0 )
	f15_local2.state = MP.ExtractBitsFromBitPositionInValue( f15_local3, 2, 1 )
	f15_local2.codeLength = MP.ExtractBitsFromBitPositionInValue( f15_local3, 4, 3 )
	f15_local2.codeDigits = MP.ExtractBitsFromBitPositionInValue( f15_local3, 16, 7 )
	return f15_local2
end
...
BR.PlayAgainAction = function ( f14_arg0, f14_arg1 )
	...
	elseif Lobby.IsNotAloneInPrivateParty() then
		if CONDITIONS.IsPlayAgainEnabledForPartyHost() then
			...
			ACTIONS.OpenPopupMenu( f14_arg0, "PlayAgainPartyHostPopup", true, f14_arg1 )
		elseif CONDITIONS.IsPlayAgainEnabled( f14_arg0 ) then
			DebugPrint( "------ Play Again State: party member" )
			ACTIONS.OpenPopupMenu( f14_arg0, "PlayAgainPartyMemberPopup", true, f14_arg1 )
		end
	else
		DebugPrint( "------ Play Again State: not in a party" )
		ACTIONS.OpenPopupMenu( f14_arg0, "PlayAgainPopup", true, f14_arg1 )
	end
end

BR.PlayAgainWithSquadAction = function ( f13_arg0, f13_arg1 )
	if not CONDITIONS.IsPlayAgainAllowed() then
		return 
	else
		Engine.EBHIFJCGBH( "play_again_squad_request", tonumber( Game.DBACJGFHDD( f13_arg1 ) ) )
	end
end

BR.GetInflationModeRedeployAmount = function ()
	if CONDITIONS.IsBRInflation() then
		local f12_local0 = Dvar.CFHDGABACF( "scr_br_alt_mode_inflation_cost" )
		if f12_local0 then
			local f12_local1 = tonumber( f12_local0 )
			if f12_local1 then
				return f12_local1 * BRCONTRACTS.PLUNDER_MULTIPLIER
			end
		end
	end
	return -1
...
BR.GetNumTierSkips = function ( f10_arg0 )
	return ScheduledChallenges.HCHJBFIEH( f10_arg0 )
end

BR.GetNumChallengesBanked = function ( f9_arg0 )
	local f9_local0 = ScheduledChallenges.CCECJEFJDH( f9_arg0 )
	local f9_local1
	if f9_local0 then
		f9_local1 = #f9_local0
...
BR.SetPlayAgainChoice = function ( f8_arg0 )
	local f8_local0 = assert
	local f8_local1
	if 0 > f8_arg0 or f8_arg0 > BR.PlayAgainChoice.WITH_PARTY then
		f8_local1 = false
	else
		f8_local1 = true
	end
	f8_local0( f8_local1 )
	Dvar.DFIJDJFIFD( "LSORRMTOPP", f8_arg0 )
end

BR.AbortPlayAgain = function ( f7_arg0 )
	Engine.EBIDFIDHIE( f7_arg0 )
end

BR.ExecuteBREndTransition = function ( f6_arg0, f6_arg1, f6_arg2 )
	Engine.BEFHEEBHIA()
	local f6_local0 = f6_arg0:getRootParent()
	if f6_local0.squadEliminatedLayer then
		LUI.UIRoot.SetLuiLayerKeycatch( f6_local0.squadEliminatedLayer, f6_arg1, false )
	end
	if CONDITIONS.IsBRQuarryTutorialMap() and f6_arg2 == "map_restart" then
		Engine.EBHIFJCGBH( "exit_squad_eliminated", BR.ExitGameCommandType.MAP_RESTART )
		return 
	else
		Engine.EBIDFIDHIE( f6_arg2 )
	end
end

BR.SetupEndgameDataSource = function ( f5_arg0 )
	DataSources.inGame.BR.BREndGameTime = LUI.DataSourceInControllerModel.new( "alwaysLoaded.brEndGameTime" )
	DataSources.inGame.BR.BREndGameTime:SetValue( f5_arg0, tonumber( Engine.DCJHCAFIIA() ) + BR.END_GAME_TIMER_LENGTH_SECONDS )
end

BR.GetRemainingEndgameTime = function ( f4_arg0 )
	local f4_local0 = DataSources.inGame.BR.BREndGameTime
	if not f4_local0 then
		BR.SetupEndgameDataSource( f4_arg0 )
		f4_local0 = DataSources.inGame.BR.BREndGameTime
	end
	local f4_local1 = tonumber( Engine.DCJHCAFIIA() )
	local f4_local2 = f4_local0:GetValue( f4_arg0 )
	if 0 < f4_local2 - f4_local1 then
		return Engine.JCBDICCAH( Engine.BJCFFHAIFA( f4_local2 - f4_local1 ) )
	else
		
	end
end

BR.GetTotalTeamCash = function ( f3_arg0 )
	local f3_local0 = Game.BFEEGCHICA( f3_arg0, "ui_br_team_cash_banked" ) or 0
	local f3_local1 = Game.BFEEGCHICA( f3_arg0, "ui_br_team_cash_pockets" ) or 0
	local f3_local2 = 0
	if f3_local0 ~= nil and f3_local1 ~= nil then
		...
		if CONDITIONS.IsRiskGametype() then
			...
			f3_local2 = f3_local2 * BRLOOT.TEAM_CASH_SCALAR
		end
	end
	return f3_local2
end

BR.GetPrestigeStorePopupData = function ( f1_arg0 )
	return {
