...
local f0_local0 = function ( f11_arg0, f11_arg1, f11_arg2 )
	assert( f11_arg2 )
	if Engine[0xC436AD5B92B0408]( f11_arg1 ) then
		LUI.FlowManager.RequestPopupMenu( f11_arg0, "OverwriteSaveWarningPopup", true, f11_arg1, false, {
			mapName = f11_arg2
		} )
	else
		local f11_local0 = MenuBuilder.BuildRegisteredType( "SPPauseDifficultyFlyin", {
			controllerIndex = f11_arg1
		} )
		f11_local0:SetMap( f11_arg2 )
...
local f0_local1 = function ( f6_arg0, f6_arg1 )
	...
	if not Dvar[0x9E5BE3B4BBA4E0E]( 0x5933EFE2016FFFA ) then
		...
		for f6_local4 = 0, f6_local3, 1 do
			...
			local f6_local8 = f6_local7.name
			if f6_local7.previewBack == nil or f6_local7.previewBack == "" then
				f6_local7.previewBack = "white"
			end
			if f6_local7.previewFront == nil or f6_local7.previewFront == "" then
				f6_local7.previewFront = "white"
			end
			local f6_local9 = {
				disabled = true,
			...
			if f6_local9.missionState == "complete" then
				f6_local1 = f6_local4
			end
			local f6_local10 = f6_local4 - 1 <= f6_local1
			local f6_local11
			if f6_local9.missionState == "locked" then
		...
		for f6_local4 = 0, f6_local3 - 1, 1 do
			...
			local f6_local8 = {
				...
				levelIndex = f6_local4,
				levelName = MapInfo.BundleGetName( f6_local7 ),
			...
			if f6_local8.missionState == "complete" then
				f6_local1 = rowIndex
			end
			local f6_local9 = f6_local1 >= rowIndex - 1
			local f6_local10
			if f6_local8.missionState == "locked" then
				f6_local10 = f6_local9
			else
				f6_local10 = true
			end
			if f6_local10 or SPSharedUtils.UnlockAllMissions( f6_arg1 ) then
				if f6_local8.isCurrentMission then
					f6_local0 = #f6_arg0._levelList
				end
				f6_local8.disabled = false
				table.insert( f6_arg0._levelList, f6_local8 )
			end
		end
	end
		...
		if not f7_arg0.addHandlers then
			...
			f7_arg0:registerEventHandler( "button_action", function ( element, event )
				SPSharedUtils.ShaderPreloadWrap( function ()
					...
					assert( element )
					local f9_local0 = element
					if f9_local0.isCurrentMission then
						LUI.FlowManager.RequestPopupMenu( element, "ResumeOrRestartPopup", false, element.controller, false, {
							...
							mapName = f9_local0.levelName,
							levelName = f9_local0.displayName
						} )
					else
						element( element, element.controller, f9_local0.levelName )
					end
				end )
			end )
		end
...
function CampaignMissionSelect( menu, controller )
	...
	if not MenuTitle.bindButton then
		...
		f1_local5.id = "MenuTitleBindButton"
		MenuTitle:addElement( f1_local5 )
		MenuTitle.bindButton = f1_local5
	end
	MenuTitle.bindButton:addEventHandler( "button_secondary", function ( f2_arg0, f2_arg1 )
