...
MP_COMMON.StartPrivateMatchLobby = function ( f15_arg0, f15_arg1, f15_arg2 )
	if f15_arg2 ~= nil then
		Dvar[0x2414681ECAB880F]( 0x18C699A5CAAEE4F, f15_arg2 )
	end
	Dvar[0x2414681ECAB880F]( 0x2FFBD81B5FEF078, f15_arg1 )
	MP_COMMON.StartPrivateMatchLobbyDs( f15_arg0 )
end

MP_COMMON.StartPrivateMatchLobbyDsRequired = function ( f14_arg0, f14_arg1 )
	MP_COMMON.StartPrivateMatchLobby( f14_arg0, Enum[0x870710B3AC3F3C9][0x1DD3CEAFC43A598], f14_arg1 )
end

MP_COMMON.StartPrivateMatchLobbyDsOptional = function ( f13_arg0, f13_arg1 )
	MP_COMMON.StartPrivateMatchLobby( f13_arg0, Enum[0x870710B3AC3F3C9][0xB30D3FC90392A8E], f13_arg1 )
end

MP_COMMON.StartPrivateMatchLobbyDsNone = function ( f12_arg0, f12_arg1 )
	MP_COMMON.StartPrivateMatchLobby( f12_arg0, Enum[0x870710B3AC3F3C9][0x80A6307B27393DA], f12_arg1 )
end
...
MP_COMMON.SetupStreaming = function ( f8_arg0, f8_arg1 )
	...
	f8_arg0:addElement( MPLobbyHints.GetMPMapPreloadStreamer( f8_arg1 ) )
	MP_COMMON.LoadStreamingModels( f8_arg0, f8_arg1, LOADOUT.GetFavoriteLoadoutIndex( f8_arg1 ) )
end

MP_COMMON.LoadStreamingModels = function ( f7_arg0, f7_arg1, f7_arg2, f7_arg3 )
	local f7_local0 = LOADOUT.GetLoadoutSquadMember( f7_arg1 )
	f7_local0 = f7_local0.loadouts
	if not f7_arg3 then
		local f7_local1 = {}
		for f7_local2 = 0, 1, 1 do
			local f7_local5 = f7_local0[f7_arg2].weaponSetups[f7_local2]
		...
		if 0 < #f7_local1 then
			STREAMING.RequestModels( f7_arg1, {
				requesterWidget = f7_arg0,
	...
	for f7_local3 = 0, 1, 1 do
		local f7_local6, f7_local8 = OPERATOR.GetEquippedOperatorSkin( f7_arg1, f7_local3 )
...
MP_COMMON.TriggerLeaveMainGamemodeMenu = function ( f6_arg0, f6_arg1 )
	...
	if CONDITIONS.IsLFPEnabled() then
		LFP.StopSearchAndClearPreferences( f6_arg1.controller )
	end
	if Lobby.IsNotAloneInPrivateParty() and f6_local0 then
		if Engine[0x688D9FAA39ECE94]() then
			LUI.FlowManager.RequestPopupMenu( f6_arg0, MP_COMMON.RATE_LIMITED_LEAVE_MAIN_GAMEMODE_MENU_POPUP_NAME, true, f6_arg1.controller, false, {
				rateLimitIntervalMS = 5000,
				rateLimitAttempts = 3,
				menu = f6_arg0
			} )
		elseif PARTY.IsLocalPlayerPartyQueuing() then
			PARTY.LeaveParty( f6_arg0, f6_arg1.controller, Social[0xBE9B627D9C821F6]() )
		else
			LUI.FlowManager.RequestPopupMenu( f6_arg0, MP_COMMON.LEAVE_MAIN_GAMEMODE_MENU_POPUP_NAME, true, f6_arg1.controller, false, {
				menu = f6_arg0
			} )
		end
	else
