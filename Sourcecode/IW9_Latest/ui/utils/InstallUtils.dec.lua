...
Install = {
	...
	IsDiscCheckRequired = function ( f15_arg0, f15_arg1 )
		if CONDITIONS.IsPS5() or CONDITIONS.IsPC() and CONDITIONS.IsDevelopmentBuild() then
			...
			if f15_arg1 then
				...
				if not f15_local3 then
					...
					if not f15_local4 then
						DebugPrint( "ERROR Install.IsDiscCheckRequired - Invalid controllerIndex - Skipped disc check!" )
					end
					local f15_local5
					if f15_local4 then
					...
					if f15_local5 then
						f15_local6 = CONDITIONS.IsEntitlementOnDisk( PLAY_MENU.GAME_REFS.MODERN_WARFARE_II )
					end
					return f15_local6
				end
			end
