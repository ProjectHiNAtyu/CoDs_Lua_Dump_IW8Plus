...
local f0_local21 = function ( f55_arg0, f55_arg1 )
	...
	if not Engine.DDJFBBJAIG() then
		if f55_local0.openedForPlunderBox == true or CONDITIONS.IsBRLoadoutGameType( f55_arg1 ) then
...
local f0_local25 = function ( f43_arg0, f43_arg1, f43_arg2, f43_arg3, f43_arg4, f43_arg5 )
	...
	if CONDITIONS.InGame() then
		...
		if not f43_arg5 and not Engine.BGGJFFFAFG( f43_arg1 ) and not CONDITIONS.InGameAndBRLoadoutSelect( f43_arg1 ) then
	...
	if not Engine.DDJFBBJAIG() then
		...
		if f43_local0.state == f0_local0.mix then
			...
			if not f43_local6 then
				f43_local6 = CONDITIONS.IsBRLoadoutGameType( f43_arg1 )
			end
			f43_local4( f43_local3, f43_local5, f43_local6 )
		end
	end
	local f43_local4 = 0
	if CONDITIONS.InGameAndBRLoadoutSelect( f43_arg1 ) and not f43_local1:IsDisabled() then
		...
		if not f43_local6 then
			...
			if not f43_local1.handlersAdded then
				f43_local8 = function ( f48_arg0, f48_arg1 )
					...
					if not Engine.DDJFBBJAIG() then
						if not f48_arg0.isDefault and f43_arg0.openedForPlunderBox ~= true and not CONDITIONS.IsBRLoadoutGameType( f43_arg0 ) and not (Dvar.IBEGCHEFE( "scr_br_disableLoadoutEdit" ) or false) then
			...
			if CONDITIONS.IsBRLoadoutGameType( f43_arg1 ) and f43_arg2.LoadoutButton and f43_arg2.LoadoutButton.BRWarning then
				if f43_arg5 then
...
local f0_local27 = function ( f23_arg0, f23_arg1, f23_arg2 )
	...
	if not Engine.DDJFBBJAIG() and CONDITIONS.IsMagmaGameMode() and Game.BFEEGCHICA( f23_arg1, "ui_open_loadout_bag" ) then
		f23_local2 = true
	end
	...
	if not f23_local3.cacSessionDlogOnGoing and not CONDITIONS.IsBRLoadoutGameType( f23_arg1 ) then
	...
	elseif f23_local3.menuState ~= f0_local0.loadoutEdit then
		if f23_local3.openedForPlunderBox == true or CONDITIONS.IsBRLoadoutGameType( f23_arg1 ) then
