...
local f0_local0 = function ( f10_arg0, f10_arg1 )
	...
	Engine.DAGFFDGFII( "uploadstats", f10_arg1 )
	if not Engine.DAFGFCFHJI() then
		Lobby.BHDJFHEDJG( f10_arg1 )
	end
...
local f0_local1 = function ( f9_arg0, f9_arg1 )
	...
	Engine.DAGFFDGFII( CPConfig.default_aliens, f9_local0 )
	...
	if f9_local1 then
		Lobby.BHDJFHEDJG( f9_local0 )
	end
	if CONDITIONS.IsSeasonalEventActive() then
		local f9_local2 = SEASONAL_EVENT.GetCurrentSeasonalEventOnboadingData()
		if f9_local2 and not f9_local2.WasCompleted( f9_arg0, f9_local0 ) then
			local f9_local3 = LUI.FlowManager.IsInStack( "SeasonalEventIntroPopup" )
			if not f9_local3 then
				f9_local3 = LUI.FlowManager.IsInStack( "SeasonalEventDetailsPopup" )
				if not f9_local3 then
					f9_local3 = LUI.FlowManager.IsInStack( "SeasonalEventSpecialPopup" )
				end
			end
			if f9_local3 then
				return 
			end
			local f9_local4 = SEASONAL_EVENT.GetCurrentSeasonalEventData()
			if f9_local4.onboardingBinkKorea15Unsafe and Engine.FABABBDBA() then
				LUI.FlowManager.RequestPopupMenu( f9_arg0, "SeasonalEventDetailsPopup", true, f9_local0, false, {} )
			else
				LUI.FlowManager.RequestPopupMenu( f9_arg0, "SeasonalEventIntroPopup", true, f9_local0, false, {} )
			end
			return 
		end
	end
	local f9_local2 = LUI.DataSourceInControllerModel.new( "alwaysLoaded.openedSeasonalFlow" )
	if not f9_local2:GetValue( f9_local0 ) then
		...
		elseif BATTLEPASS.ShouldShowFreeTierUpsell( f9_local0 ) then
			LUI.FlowManager.RequestPopupMenu( f9_arg0, "BattlePassFreeTierUpsell", true, f9_local0, false, {
				sourceMenu = f9_arg0
			} )
			return 
		end
		DYN_ATTACHMENT.ReloadLoadoutWeaponStreamedImages( f9_arg0, f9_local0 )
		if not LUI.WebViewManager.MotdEnabled() then
			CRM.OpenCRMWindows( f9_arg0 )
		end
		f9_arg0._canOpenDailyPopup = true
	end
end
