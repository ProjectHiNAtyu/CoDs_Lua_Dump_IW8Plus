	...
	if f3_local1 then
		...
		if f3_local1._currentCursorType ~= f3_local0 then
			...
			if Engine.ECADDGAFF() then
				...
				local f3_local2 = RegisterMaterial
				local f3_local3 = f3_local0.material
				if not f3_local3 then
					f3_local3 = MouseCursorType.CURSOR_TYPE_ARROW_NORMAL.material
				end
				f3_local2 = f3_local2( f3_local3 )
				assert( f3_local2, "Cursor material " .. tostring( f3_local0.material ) .. " invalid or not loaded" )
				f3_local1:setImage( f3_local2 )
			end
		end
	end
end
