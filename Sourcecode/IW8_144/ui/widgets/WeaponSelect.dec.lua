...
local f0_local8 = function ( f47_arg0, f47_arg1, f47_arg2, f47_arg3, f47_arg4 )
	return function ( f48_arg0, f48_arg1 )
		...
		if not f48_local0 then
			if f47_arg0.weaponData.unlockThroughSeasonalEvent then
				f48_local4.unlockText = f47_arg0.weaponData.unlockCriteria
				f48_local4.unlockThroughSeasonalEvent = true
			else
	...
	if f31_local0.weaponFilterType and f31_local0.weaponFilterType == LOOT.weaponFilterOptions.ALL then
		...
		for f31_local3 = 1, f31_local2, 1 do
			for f31_local10, f31_local11 in pairs( f31_arg0._weaponTable[f31_arg0.tabsData[f31_local3].classRef] ) do
				...
				if f31_local0.weaponFilterType == LOOT.weaponFilterOptions.VANGUARD and f31_local11.gameSourceID == LUI.GAME_SOURCE_ID.VANGUARD then
					f31_local1[f31_local3] = true
				end
			end
		end
...
local f0_local18 = function ( f30_arg0, f30_arg1 )
	...
	for f30_local5, f30_local6 in pairs( f30_arg0._weaponTable[f30_arg0.tabsData[f30_arg0._currentTabIndex].classRef] ) do
		...
		if f30_local1.weaponFilterType == LOOT.weaponFilterOptions.VANGUARD then
			if f30_local6.gameSourceID == LUI.GAME_SOURCE_ID.VANGUARD then
				table.insert( f30_local0, f30_local6 )
			end
		end
		table.insert( f30_local0, f30_local6 )
	...
	f30_local6 = f30_local1.weaponSortingByWeaponDataType == LOOT.weaponSortByWeaponDataOptions.NEW_FIRST
...
local f0_local22 = function ( f13_arg0, f13_arg1, f13_arg2 )
	...
	if f13_local5 then
		...
		if not f13_local6 then
			...
			local f13_local9 = SEASONAL_EVENT.GetData( f13_arg1 )
			f13_arg0._weaponTable = {}
			local f13_local8 = {}
			for f13_local10 = 0, StringTable.BFHCAIIDA( CSV.weapons.file ), 1 do
				local f13_local13 = CSV.ReadRow( CSV.weapons, f13_local10 )
				local f13_local14 = tonumber( f13_local13.displayOrder )
				if f13_local14 then
					f13_local14 = 0 <= tonumber( f13_local13.displayOrder )
				end
				local f13_local15 = Engine.BHICADFIHA()
				if not f13_local15 then
					if #f13_local13.UIShowDvar ~= 0 then
						f13_local15 = Dvar.IBEGCHEFE( f13_local13.UIShowDvar )
					else
						f13_local15 = true
					end
				end
				if f13_local14 and f13_local15 then
					local f13_local16 = MatchRules.CADFFHGJAE( f13_local13.ref )
					if not CONDITIONS.InGame() or not f13_local16 then
						if not f13_arg0._weaponTable[f13_local13.menuClass] then
							f13_arg0._weaponTable[f13_local13.menuClass] = {}
						end
						f13_local13.isUnlocked = PROGRESSION.IsUnlocked( f13_arg1, LOOT.itemTypes.WEAPON, f13_local13.ref )
						local f13_local17
						if f13_local13.isUnlocked or f13_local13.hiddenWhenLocked ~= "1" then
							f13_local17 = false
						else
							f13_local17 = true
						end
						local f13_local18 = LOOT.GetItemID( LOOT.itemTypes.WEAPON, f13_local13.ref )
						local f13_local19 = not LOOT.IsItemRefEquippableForMode( LOOT.itemTypes.WEAPON, f13_local13.ref )
						local f13_local20
						...
						if not f13_local13.isUnlocked then
							local f13_local21 = weaponModel[f13_local18]
							local f13_local22 = false
							if CONDITIONS.IsSeasonalEventActive() then
								for f13_local26, f13_local27 in ipairs( f13_local9 ) do
									if tonumber( f13_local27.lootID ) == f13_local18 then
										f13_local22 = true
										break
									end
								end
							...
							if f13_local21 then
								f13_local13.unlockCriteria = Engine.CBBHFCGDIC( "BATTLEPASS/LOCKED_INCOMPLETE_WITH_BATTLEPASS", f13_local21 )
							elseif f13_local22 then
								f13_local13.unlockCriteria = Engine.CBBHFCGDIC( "SEASONAL_EVENT/COMPLETE_ALL_GAME_CHALLENGES", #f13_local9 / 2 - 1, Engine.CBBHFCGDIC( "SEASONAL_EVENT/SEASONAL_EVENT_TITLE" ) )
								f13_local13.unlockThroughSeasonalEvent = true
							else
								f13_local13.unlockCriteria = PROGRESSION.GetUnlockText( f13_arg1, LOOT.itemTypes.WEAPON, f13_local13.ref )
							end
						end
						if not f13_local20 then
							if CONDITIONS.UsingWZLoadouts() then
								f13_local13.lootID = f13_local18
								f13_local13.gameSourceID = LOOT.GetGameSourceIDFromItemID( f13_local18 )
							end
							f13_local13.isRestricted = f13_local16
							if f13_local16 then
								if not f13_local8[f13_local13.menuClass] then
									f13_local8[f13_local13.menuClass] = {}
								end
								table.insert( f13_local8[f13_local13.menuClass], f13_local13 )
							end
							table.insert( f13_arg0._weaponTable[f13_local13.menuClass], f13_local13 )
						end
					end
				end
			end
			for f13_local28, f13_local13 in pairs( f13_arg0._weaponTable ) do
				table.sort( f13_local13, function ( f19_arg0, f19_arg1 )
					return tonumber( f19_arg0.displayOrder ) < tonumber( f19_arg1.displayOrder )
				end )
			end
			for f13_local28, f13_local13 in pairs( f13_local8 ) do
				table.sort( f13_local13, function ( f18_arg0, f18_arg1 )
					return tonumber( f18_arg0.displayOrder ) < tonumber( f18_arg1.displayOrder )
				end )
			end
			for f13_local28, f13_local13 in pairs( f13_local8 ) do
				for f13_local17, f13_local18 in pairs( f13_local13 ) do
					table.insert( f13_arg0._weaponTable[f13_local28], f13_local18 )
				end
			end
			...
			f13_local10 = {}
			for f13_local11 = 0, StringTable.BFHCAIIDA( CSV.weaponClasses.file ), 1 do
				local f13_local14 = CSV.ReadRow( CSV.weaponClasses, f13_local11 )
				if f13_local14.showInMenus == "1" and tonumber( f13_local14.slot ) == f13_local6 + 1 and f13_arg0._weaponTable[f13_local14.ref] then
					local f13_local15
					if f13_local14.isWZOnly == "1" then
						f13_local15 = not CONDITIONS.UsingWZLoadouts()
					else
						f13_local15 = false
					end
					if not f13_local15 then
						if not CONDITIONS.InGame() or not MatchRules.DEJJEDIGDE( f13_local14.ref ) then
							local f13_local17 = {
								classRef = f13_local14.ref,
								name = Engine.CBBHFCGDIC( f13_local14.pluralName ),
								...
								weaponData = f13_arg0._weaponTable[f13_local14.ref]
							}
							if f13_local14.ref == WEAPON.GetWeaponClass( f13_arg0.equippedWeapon ) then
								f13_local17.hasDefaultFocus = true
							end
							table.insert( f13_local10, f13_local17 )
						end
					end
				end
			end
			f13_arg0.tabsData = f13_local10
			f13_arg0.Tabs:SetTabs( f13_local10 )
			f13_arg0.Tabs:SetAlignment( LUI.Alignment.Left )
			for f13_local11 = 1, #f13_local10, 1 do
				f13_arg0:UpdateTabBreadcrumb( f13_local11 )
			end
