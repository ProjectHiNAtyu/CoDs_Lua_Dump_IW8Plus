...
local f0_local2 = function ( f10_arg0, f10_arg1, f10_arg2, f10_arg3 )
	...
	local f10_local0 = function ( f11_arg0 )
		...
		if f10_arg0 then
			f11_local0 = WEAPON.GetWeaponClass( WEAPON.StripSuffix( f10_arg0, "_mp" ) )
			if not f11_local0 or f11_local0 == "" then
				...
				if f11_local3 then
					local f11_local4 = WEAPON.GetWeaponDataForAssetRef( string.sub( f11_local2, 0, f11_local3 + 2 ) )
					if f11_local4 then
						local f11_local5 = f11_local4[0x11655952FCBAB9F]
			...
			f11_local0 = DataModel[0x614D394F6F9A18D]( f11_arg0 )
		end
		if f10_arg0.currentWeaponClass ~= f11_local0 then
			f10_arg0.currentWeaponClass = f11_local0
			local f11_local6 = BRLOOT.weaponClassData[f11_local0]
			if f11_local6 then
				f11_local6 = BRLOOT.weaponClassData[f11_local0].ammoData
			end
			if f11_local6 then
				f10_arg0.AmmoImage:setImage( RegisterMaterial( f11_local6.ammoImage ) )
				local f11_local1 = DataSources.alwaysLoaded.player.currentWeapon.isDualWielding
				local f11_local2 = DataSources.alwaysLoaded.player.currentWeapon.ammoWarningStateLeft
				local f11_local3 = DataSources.alwaysLoaded.player.currentWeapon.ammoWarningStateRight
				local f11_local4 = f11_local3:GetValue( f10_arg0 )
				local f11_local5 = f11_local2:GetValue( f10_arg0 )
				local f11_local7 = nil
				if not f11_local1:GetValue( f10_arg0 ) then
					f11_local7 = f11_local4
				end
				if f11_local5 and f11_local4 then
					if math.min( f11_local5, f11_local4 ) == f11_local5 and not f11_local2 then
				...
				if f11_local7 then
					f10_arg0:SetAmmoCountDS( f11_local7:GetModel( f10_arg0 ), f11_local6.maxAmmo )
				end
			else
...
function BRAmmoTypeNoCount( menu, controller )
	...
	self._sequenceLengths.HasAmmo = 0
	local f1_local6 = nil
	...
	self._sequenceLengths.NoWeapon = 0
	local f1_local8 = nil
	...
	self._sequenceLengths.MaxAmmo = 0
