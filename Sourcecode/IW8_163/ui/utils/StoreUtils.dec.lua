...
STORE.GetAllItemsInStore = function ( f34_arg0, f34_arg1 )
	...
	for f34_local9, f34_local10 in ipairs( STORE.GetCategoryData( f34_arg0 ) ) do
		f34_local10.items = f34_local10.items or {}
		local f34_local11 = f34_local10.items and f34_local10.items[1] or {}
		if not f34_local11.emptyCategory and f34_local10.idPubVar and f34_local11.layoutOnlyItem then
			local f34_local4 = STORE.GetCategoryItemIDs( f34_arg0, f34_local10 )
			for f34_local5 = 1, #f34_local4, 1 do
				local f34_local8 = f34_local4[f34_local5]
				if STORE.BundleAvailable( f34_arg0, f34_local8 ) and not STORE.IsFirstPartyBundle( f34_local8 ) then
					f34_arg1.AddItemsFunction( f34_arg0, f34_local8, f34_local0, f34_arg1 )
				end
			end
		end
	end
...
STORE.GetCategoryData = function ( f29_arg0, f29_arg1 )
	assert( f29_arg0 )
	local f29_local0 = f29_arg1
	if not f29_arg1 then
		f29_local0 = Engine.DBFHBBIDHD( Store.DBFHBBIDHD() )
	end
	if Dvar.IBEGCHEFE( "TNMTRNOQT" ) then
		...
		if f29_local1 and not STORE.GetCategoryIndexWithFanchiseItems( f29_local0, STORE.ZeusCategoryID ) and 0 < #STORE.GetZeusCategoryData( f29_arg0 ) then
			table.insert( f29_local0, STORE.GetZeusCategoryIndex(), {
	...
	if Dvar.IBEGCHEFE( "online_store_x2_category_enabled" ) then
		...
		if STORE.IsX2Owned( f29_arg0 ) ~= false then
			f29_local1 = STORE.IsVanguardUpgradeToUltimateExisteAndNotOwned( f29_arg0 )
		else
			f29_local1 = true
		end
		if f29_local1 and not STORE.GetCategoryIndexWithFanchiseItems( f29_local0, STORE.X2CategoryID ) and 0 < #STORE.GetVanguardCategoryData( f29_arg0 ) then
			table.insert( f29_local0, STORE.GetX2CategoryIndex(), {
	...
	if Dvar.IBEGCHEFE( "LMMPLKNTMP" ) then
		local f29_local1
		if STORE.IsCortezOwned( f29_arg0 ) ~= false then
			f29_local1 = STORE.IsCortezUpgradeToUltimateExistentAndNotOwned( f29_arg0 )
		else
			f29_local1 = true
		end
		if f29_local1 and not STORE.GetCategoryIndexWithFanchiseItems( f29_local0, STORE.CortezCategoryID ) and 0 < #STORE.GetCortezCategoryData( f29_arg0 ) then
			table.insert( f29_local0, STORE.GetCortezCategoryIndex(), {
