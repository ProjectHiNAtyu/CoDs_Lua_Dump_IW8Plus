...
FenceGamerProfile.Start = function ( f11_arg0 )
	...
	if not Engine[0x7B17D94988FD5D0]( "hasEverPlayed_MainMenu" ) then
...
FenceGamerProfile.ProcessingEULA = function ( f7_arg0, f7_arg1 )
	if CONDITIONS.IsCIABuildEnabled() and not LUI.WebViewManager.WebEULAEnabled() then
		return true
	elseif LUI.WebViewManager.WebEULAEnabled() then
		if Dvar[0x9E5BE3B4BBA4E0E]( 0xFB51FDBE670A15C ) then
			local f7_local0 = LUI.WebViewManager.Instance()
			f7_local0:MakeSureEULAClosed()
			return false
		elseif not Dvar[0x9E5BE3B4BBA4E0E]( 0xFB51FDBE670A15C ) and f7_arg0._openedWebEULA and not LUI.FlowManager.IsInStack( "FullScreenWebView" ) then
			f7_arg0._state = LUI.Fence.STATE.fail
			return true
		end
		local f7_local0 = LUI.WebViewManager.Instance()
		if not f7_local0:SystemIsReady() then
			f7_local0 = LUI.WebViewManager.Instance()
			f7_local0:UpdateTelescopeRequirement( true )
			if not LUI.FlowManager.IsInStack( "LoadingEulaFencePopup" ) then
				LUI.FlowManager.RequestPopupMenu( nil, "LoadingEulaFencePopup", false, f7_arg1, false, {}, nil, true, true )
			end
		...
		if not f7_arg0._openedWebEULA then
			f7_local0 = LUI.WebViewManager.Instance()
			f7_arg0._openedWebEULA = f7_local0:OpenEULA()
		end
		f7_arg0._state = LUI.Fence.STATE.block
		return true
	elseif not Engine[0x7B17D94988FD5D0]( "acceptedEULA", f7_arg1 ) and (not CONDITIONS.IsMobileOrMobileSim() or CONDITIONS.IsMobileLegalEnable()) then
		f7_arg0._state = LUI.Fence.STATE.block
...
FenceGamerProfile.UpdateState = function ( f6_arg0 )
	...
	if not CONDITIONS.IsMobileOrMobileSim() and Engine[0x5575DCFF312365A]() and not Engine[0x20B186F60BAEF07]() then
		return 
	elseif f6_local0 and Engine[0xD2813B521711C11]( f6_local0 ) and not Engine[0x3CE55FFE2E82102]( f6_local0 ) then
		local f6_local1 = Engine[0x961ADE0742DBB36]( f6_local0 )
		local f6_local2 = Engine[0xF35D89342CF7712]()
		if CONDITIONS.IsCIABuildEnabled() or Dvar[0x9E5BE3B4BBA4E0E]( 0xBD5EF171527B9C ) or Dvar[0x9E5BE3B4BBA4E0E]( 0xA0D285D5AF3D531 ) and not Engine[0x7B17D94988FD5D0]( "acceptedEULA", f6_local0 ) then
		...
		elseif not Engine[0x575E9C5B9AAB6F4]() and not Engine[0xE25E29D8ADE8F27]() and f6_local1 < #Languages and f6_local2 ~= f6_local1 then
		...
		elseif not Engine[0x7B17D94988FD5D0]( "hasEverPlayed_MainMenu", f6_local0 ) and not CONDITIONS.IsMobileOrMobileSim() then
			...
			if IsLanguageSubtitleOnByDefault() then
				Engine[0xC29F5E64D83F115]( f6_local0, true )
			end
		end
	end
end
