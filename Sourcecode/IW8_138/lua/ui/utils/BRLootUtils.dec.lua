...
BRLOOT.ValidateImage = function ( f13_arg0 )
	...
	if not f13_arg0 then
		f13_local0 = BRLOOT.ERROR_TEXTURE
...
BRLOOT.InitLootCSV = function ()
	...
	for f12_local4, f12_local5 in pairs( BRLOOT.ammoTypes ) do
		local f12_local6 = tonumber( StringTable.DIFCHIGDFB( CSV.BRLootBase.file, CSV.BRLootBase.cols.name, f12_local5, CSV.BRLootBase.cols.maxStackSize ) )
	...
	f12_local0 = tonumber( StringTable.DIFCHIGDFB( CSV.BRLootBase.file, CSV.BRLootBase.cols.name, "brloot_armor_plate", CSV.BRLootBase.cols.classCharacteristicTitle ) )
	f12_local1 = BRLOOT
	if not f12_local0 then
		f12_local2 = f12_local0
	else
		f12_local2 = BRLOOT.MAX_ARMOR
	end
	f12_local1.MAX_ARMOR = f12_local2
	f12_local1 = tonumber( StringTable.DIFCHIGDFB( CSV.BRLootBase.file, CSV.BRLootBase.cols.name, "brloot_armor_plate", CSV.BRLootBase.cols.maxStackSize ) )
	f12_local2 = BRLOOT
	if not f12_local1 then
		f12_local4 = f12_local1
	else
		f12_local4 = BRLOOT.MAX_ARMOR_PLATES_HELD
	end
	f12_local2.MAX_ARMOR_PLATES_HELD = f12_local4
end

BRLOOT.ShouldShowLootCard = function ( f11_arg0 )
	return DataModel.JJEHAEBDF( DataSources.inGame.HUD.hints.interactiveObject.lootCard.shouldShow:GetModel( f11_arg0 ) ) == 1
end

BRLOOT.IsValidRarityNum = function ( f10_arg0 )
	local f10_local0
	if f10_arg0 < BRLOOT.itemRarities.COMMON or f10_arg0 >= BRLOOT.itemRarities.COUNT then
		f10_local0 = false
	else
		f10_local0 = true
	end
...
BRLOOT.UpdateLootCard = function ( f9_arg0, f9_arg1, f9_arg2 )
	...
	if BRLOOT.ShouldShowLootCard( f9_arg1 ) and not CONDITIONS.IsPlayerAZombie( f9_arg1 ) then
		local f9_local0 = Game.DFFJEIIAD( f9_arg1 )
		if f9_local0 then
