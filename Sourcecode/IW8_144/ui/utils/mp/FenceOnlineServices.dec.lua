...
FenceOnlineServices.OpenSummaryPopup = function ( f27_arg0 )
	if not f27_arg0._openedSummary and not LUI.FlowManager.IsInStack( "CODAccountRestartingPopup" ) then
			...
			okButtonHandler = function ( f28_arg0, f28_arg1 )
				...
				if f27_arg0._summaryController then
					...
					if Engine.BECCFCBIAA( "hasEverSeen_CODAccountOTP", f27_arg0._summaryController ) ~= 1 then
						if not Dvar.IBEGCHEFE( "MPNNSLPPNP" ) and CODACCOUNT.AccountRegistrationEnabled() and not Engine.DBAEJAHFGJ( f27_arg0._summaryController ) and not TwoFactorAuth.BDAFCCAEDG( f27_arg0._summaryController ) then
							f28_local0 = false
		...
		if CODACCOUNT.AccountRegistrationEnabled() and not Engine.DBAEJAHFGJ( f27_arg0._summaryController ) and not CONDITIONS.IsQABotSkipActive() then
