...
local f0_local3 = function ( f23_arg0, f23_arg1 )
	...
	if f23_local2 then
		for f23_local7, f23_local8 in ipairs( f23_local2 ) do
			if STORE.BundleAvailable( f23_arg0, f23_local8 ) and not isFirstParty then
				local f23_local6 = STORE.IsFirstPartyBundle( f23_local8 )
				if not isFirstparty then
					f23_local0[f23_local8] = CSV.ReadRow( CSV.bundleIDs, StringTable.BJJBBCJGEJ( CSV.bundleIDs.file, CSV.bundleIDs.cols.id, tostring( f23_local8 ) ) )
				end
			end
		end
	end
