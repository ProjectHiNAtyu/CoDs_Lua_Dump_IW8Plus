...
FenceGamerProfile.UpdateState = function ( f6_arg0 )
	...
	elseif f6_local0 and Engine.CDCCDDJAJJ( f6_local0 ) and not Engine.CIFGGDIGBE( f6_local0 ) then
		...
		local f6_local4 = BOOT.GetCurrentLegalBootPopupName()
		if f6_local4 then
			f6_local3 = LUI.FlowManager.IsInStack( f6_local4 )
		end
		...
		elseif Engine.BECCFCBIAA( "acceptedEULA_Version", f6_local0 ) < Dvar.CFHDGABACF( "QOMKPKKTP" ) then
			...
			if not f6_arg0._openedEULA then
				...
				local f6_local5 = BOOT.GetCurrentLegalBootPopupName()
				if f6_local5 then
					LUI.FlowManager.RequestPopupMenu( unused, f6_local5, false, f6_local0, false, {}, nil, true, true )
					f6_local3 = true
				else
		...
		elseif not Engine.BECCFCBIAA( "hasEverPlayed_MainMenu", f6_local0 ) then
			...
			f6_arg0._state = LUI.Fence.STATE.block
			local f6_local5 = LUI.FlowManager.IsInStack( "BootBrightness" )
			if not f6_local5 then
				f6_local5 = LUI.FlowManager.IsInStack( "BootHDRBlackPoint" )
				if not f6_local5 then
					f6_local5 = LUI.FlowManager.IsInStack( "BootHDRWhitePoint" )
				end
			end
			local f6_local6 = LUI.FlowManager.IsInStack( "ScreenMargins" )
			local f6_local7 = LUI.FlowManager.IsInStack( "BootOptions" )
			local f6_local8 = LUI.FlowManager.IsInStack( "BootAudioMixPreset" )
			if not f6_arg0._openedBrightnessMenu and not Engine.CGABICJHAI() then
				...
				return 
			elseif not f6_arg0._setScreenBounds and not Engine.CGABICJHAI() and not f6_local5 then
				LUI.FlowManager.RequestPopupMenu( unused, "ScreenMargins", false, f6_local0, false, {
					isBootOption = true
				}, nil, true, true )
				f6_arg0._setScreenBounds = true
				return 
			elseif not f6_arg0._openedAudioMixPreset and not f6_local5 and not f6_local6 then
				...
				return 
			elseif not f6_arg0._openedBootOptions and not f6_local8 and not f6_local5 and not f6_local6 then
				LUI.FlowManager.RequestPopupMenu( unused, "BootOptions", false, f6_local0, false, {}, nil, true, true )
				f6_arg0._openedBootOptions = true
				return 
			elseif not f6_local8 and not f6_local7 and not f6_local5 and not f6_local6 then
				f6_arg0._state = LUI.Fence.STATE.pass
				Engine.DAGFFDGFII( "profile_SetHasEverPlayed_MainMenu" )
				if not Engine.CGABICJHAI() then
					Engine.DAGFFDGFII( "updategamerprofile" )
		...
		elseif f6_local3 and Engine.CGABICJHAI() then
			f6_arg0._state = LUI.Fence.STATE.block
		end
	end
end
