...
local f0_local0 = function ( f19_arg0 )
	...
	if Engine.BGGJFFFAFG( f19_arg0._controllerIndex ) then
...
local f0_local1 = function ( f18_arg0 )
	...
	if CONDITIONS.WeaponHasAltMode( f18_arg0._controllerIndex, f18_arg0 ) then
		...
		if f18_local2 ~= -1 then
			...
			if not f18_local0 then
				...
				if f18_local2 == 0 then
					...
					elseif f18_local4 == WEAPON.WeaponClassInC.SPREAD then
						f18_local3 = Engine.CBBHFCGDIC( "HUD/UNDER_BARREL_AND_AMMO", "[{ui_underbarrel_shotgun}]", tostring( f18_local5 ) )
					elseif f18_local4 == WEAPON.WeaponClassInC.GRENADE then
						f18_local3 = Engine.CBBHFCGDIC( "HUD/UNDER_BARREL_AND_AMMO", "[{ui_underbarrel_grenade}]", tostring( f18_local5 ) )
					end
				end
				...
				if Engine.EAAHGICFBD() then
					f18_local4 = WEAPON.GetName( WEAPON.GetBaseRef( Game.DEJBCHBCIH( f18_arg0._controllerIndex ) ) )
				else
					f18_local4 = DataSources.inGame.player.currentWeapon.displayName:GetValue( f18_arg0._controllerIndex )
				end
				f18_local3 = Engine.CBBHFCGDIC( "HUD/WEAPON_SWAP", f18_local4 )
			end
			f18_arg0.TextAltModeToggleHint:UpdateInstruction( f18_local3, StripColorMarkers( Engine.JCBDICCAH( f18_local1 ) ) )
		else
			...
			f18_arg0.AltFireTypePrompt:SetText( StripColorMarkers( Engine.JCBDICCAH( f18_local1 ) ) )
...
local f0_local2 = function ( f17_arg0, f17_arg1 )
	...
	local f17_local2 = ClientWeapon.BCFAAFHJII( f17_arg1 )
	if f17_local2 then
		f17_local2 = CONDITIONS.IsHyperburstIconEnabled()
	end
	local f17_local3 = false
	if f17_local0 then
		if f17_local0 == ATTACHMENT.FireTypeIcon[ATTACHMENT.FireType.SINGLESHOT] and f17_local2 then
			f17_arg0.AltFireTypeIcons.FireType1:setImage( RegisterMaterial( ATTACHMENT.HyperburstIcon ) )
			f17_local3 = true
		else
	...
	if f17_local1 then
		if f17_local1 == ATTACHMENT.FireTypeIcon[ATTACHMENT.FireType.SINGLESHOT] and f17_local2 and not f17_local3 then
			f17_arg0.AltFireTypeIcons.FireType2:setImage( RegisterMaterial( ATTACHMENT.HyperburstIcon ) )
		else
...
function WeaponAltInstruction( menu, controller )
	...
	if not CONDITIONS.ShouldHideButtonPrompts( self ) then
		...
		f1_local3.Prompt.ActionIcon:setText( StripColorMarkers( MarkLocalized( "[F]" ) ), 0 )
		f1_local3.Prompt.KBMButtonPrompt.KeyString:setText( StripColorMarkers( MarkLocalized( "[F]" ) ), 0 )
		...
		f1_local3.Instruction:SetShadowVOffset( 0, 0 )
		f1_local3.InstructionAlternateKbm:setText( "", 0 )
		f1_local3.InstructionAlternateKbm:SetAlignment( LUI.Alignment.Center )
		f1_local3.PromptAlternateKbm.ActionIcon:setText( StripColorMarkers( LocalizeString( "[F]" ) ), 0 )
		f1_local3.PromptAlternateKbm.KBMButtonPrompt.KeyString:setText( StripColorMarkers( LocalizeString( "[F]" ) ), 0 )
	...
	if not CONDITIONS.ShouldHideButtonPrompts( self ) then
		f1_local3:SubscribeToModel( DataSources.inGame.player.currentWeapon.displayName:GetModel( f1_local1 ), function ()
			if not CONDITIONS.WeaponHasAltMode( menu, menu ) or CONDITIONS.IsSpectating( menu ) then
				ACTIONS.AnimateSequence( menu, "HideAltFire" )
			end
		end )
